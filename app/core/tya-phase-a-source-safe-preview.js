/* ============================================================
   ...
   No importa, no escribe, no activa proveedores ni produccion.
   ============================================================ */
window.CX = window.CX || {};
...
(function(){
  ...
  const fallbackSnapshot = {
    ...
    periods:[{ key:'pending', label:'HR pendiente', fullLabel:'HR pendiente', projectId:'cinepolis-pending', projectName:'Cinépolis', internalName:'Cinépolis HR pendiente', countries:{GT:0,HN:0}, total:0 }],
  ...
  const projects = data.periods.map(p => ({
    id: p.projectId || `cinepolis-${p.key}`,
    program: 'cinepolis',
    programLabel: 'Cinépolis',
    periodo: p.label,
    periodKey: p.key,
    name: p.internalName || `Cinépolis ${p.fullLabel || p.label}`,
    client:'TyA',
  ...
  const visits = (data.visits || []).map((v, idx) => ({
    ...
    honorario:v.honorario || 0,
    boleto:v.boleto || 0,
    combo:v.tipoCombo || 'Configurable por HR',
    comboAmt:v.comboAmt || 0,
  ...
  CX.data.currentProjectId = (latestPeriod && latestPeriod.projectId) || (projects[0] && projects[0].id);
  ...
  CX.data.programBase = function(p){ return (p && (p.programLabel || p.baseName)) || 'Cinépolis'; };
  CX.data.programKey = function(p){ return (p && (p.program || 'cinepolis')).toLowerCase(); };
