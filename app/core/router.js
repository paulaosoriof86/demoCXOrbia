...
CX.router = {
  mount(){
    ...
    this.buildRail(role);
    const first=CX.NAV[role].flatMap(g=>g.items).find(id=>CX.moduleEnabled(id)&&CX.roleCanAccess(role,id));
  ...
  buildRail(role){
    ...
    const nav=CX.NAV[role].map(group=>{
      const items=group.items.filter(id=>CX.moduleEnabled(id)&&CX.roleCanAccess(role,id)).map(id=>{
  ...
  nav(id){
    ...
    if(!m||!m.roles.includes(role)||!CX.moduleEnabled(id)||!CX.roleCanAccess(role,id)) return;
