...
**Estado:** `SOURCE_TRUTH_SYNCHRONIZED__I4B_PROVIDER_REACHED__TX_ORDER_ROOT_CAUSE_FIXED_SOURCE_ONLY__RETRY2_AUTH_REQUIRED__60_40`
...
`application.status.update` leía la visita después de escribir la postulación dentro de la transacción. Fix commit `1bde86e5e5b6c2084fe5c711b7a8c06d089f12f4`: todas las lecturas/validaciones de aprobación ocurren antes de `tx.set`. Verificador source-only reforzado en `e1f62c8425d0fffc62b2ba92ccdd6141b60f3be6`.

## Corrección sistémica de sincronización
Se comprobó que parte de la documentación canónica seguía en epoch 28 mientras Index/Execution State/Source Lock ya estaban en epoch 29. Se sincroniza el conjunto completo y `tools/verify-cxorbia-source-truth-sync.mjs` deja de depender de `EPOCH`/`FRONTIER` hard-codeados: desde ahora los deriva de `CXORBIA-EXECUTION-STATE.json` y exige coincidencia en todos los documentos canónicos antes de cualquier gate.

Evidencia activa: `app/docs/evidence/I4B-RETRY1-PROVIDER-TX-ORDER-SOURCE-FIX.json`.

## Siguiente frontera exacta
`NEW_AUTH_REQUIRED_I4B_SINGLE_DEV_VISIT_LIFECYCLE_E2E_WRITE_GATE_RETRY2__PROVIDER_TX_READ_ORDER_FIXED__SYNTHETIC_VISIT_ONLY` — nueva autorización necesaria porque Retry1 sí quedó consumido; mismo scope sintético y cero producción. PASS → I4-C HR bidireccional.
