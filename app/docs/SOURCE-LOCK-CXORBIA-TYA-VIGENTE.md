...
Los commits de autorización, executor, evidencia y docs/control-plane no sustituyen ningún SHA del release funcional congelado.

## F8 autorizado, no iniciado

La autorización single-use `PAULA-F8-BACKUP-RESTORE-CUTOVER-20260827-01` está registrada y no consumida. Ninguna mutación provider se inició. Existe executor acotado `tools/release/tya-f8-backup-restore-cutover-one-shot.mjs`, pero no existe en la sesión actual un canal GCP/provider autenticado seguro para invocarlo.
...
Clasificación: `EXTERNAL_TRANSPORT_OUTAGE_NO_SAFE_PROVIDER_EXECUTOR_IN_CURRENT_SESSION`; product P0=false.
