...
Este lock prevalece sobre el lock V0.4 anterior en la preparación técnica de materialización. Preserva I1/I2, historical I3 congelado, request08 consumido, counsel diferido y aceptación exclusivamente humana.
...
- QA/runner/IA/Make/Gemini/GitHub Actions/admin técnico no aceptan por usuarios.
...
El provider prepara create-only/readback para profile, Provider Registry core, legalContent y version. El command adapter permite `legal.acceptance.record` como self-scoped human-confirmed command a roles autenticados sin abrir otros writes de Shopper/Cliente; UID efectivo sigue derivado del ID token verificado por provider.
...
- Auth `0`;
- passwordResets `0`;
- historical access/reconciliation `0/0`;
- otherIdentity/HR/Rules/Storage/Make/Gemini/payment `0`;
- automaticAcceptance `false`;
- humanAcceptanceRequired `true`.

Provider source bloquea placeholders, `LEGAL_REVIEW_REQUIRED`, falso counsel, domicilio restringido público, collision/overwrite y budget drift.
...
Los valores TyA no se hardcodean en runtime. El bootstrap request puede llevar valores iniciales bajo autorización; tras provider ACK la autoridad es provider/no-code.

## 6. Evidencia CI acumulativa

- HEAD técnico `4cfd087fb49bb41d00caa9dd798bf7d02fa4f0d9`: run `31959900456`, job `95196342385`, SUCCESS.
- HEAD documental `7862a4f67fe5ce526d5e4b465e9e19bff65a28d8`: run `31960246332`, job `95197007415`, SUCCESS.
- **Checkpoint final `d50fbbd07bedca89b03c667e97ab76a830d644bc`: run `31960342757`, job `95197241342`, SUCCESS.**

En el checkpoint final pasaron I1, I2, frozen I3, legal-aware historical harness, overlay-aware Admin subgate, durable legal acceptance, immutable publication snapshot, `Verify I3 V0.4 interim materialization provider source contract` y current operational checkpoint.
...
`CXOrbia Phase A Firestore Materialization Executor` existe, pero su `execute` actual está limitado a emulator. No se confundirá con materialización real de DEV. La ejecución real debe reutilizar/extender un carril existente bajo gate exacto, sin workflow nuevo y sin request08.
