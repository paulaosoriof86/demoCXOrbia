...
3. `backend/runtime/cxorbia-project-command-provider-v1.mjs`: provider source-only para `project.create`/`project.update` con Auth/RBAC, idempotencia, versión, validación de fuente y prohibición de secretos crudos.
...
4. `PROJECT_ISOLATION_PASS` — crear proyecto B no altera visitas, periodos, fuente, cuestionario, finanzas ni usuarios del proyecto A.
5. `PROJECT_PROVIDER_BINDING_PASS` — cada proyecto externo usa su propio binding/mapping indirectos.
6. `PROJECT_INTERNAL_SOURCE_PASS` — proyecto con Hoja de Ruta interna funciona sin proveedor externo.
7. `PROJECT_EXTERNAL_SOURCE_PASS` — un proyecto externo puede descubrir periodos/visitas según su provider sin cambiar código global.
8. `PROJECT_MONTH_FUTURE_PASS` — meses/periodos nuevos se descubren desde la fuente configurada, no por lista hardcodeada.
9. `PROJECT_COMMAND_CONFLICT_PASS` — idempotencia y expectedVersion evitan duplicados/escritura perdida.
10. `PROJECT_DOCUMENTATION_SYNC_PASS` — índice/checkpoint/execution/source-lock/mirrors tienen el mismo epoch y NEXT.

## Claude/prototipo — cambios que no debe ejecutar todavía

Cuando los gates backend estén listos, Claude Code recibirá un patch focal para `app/modules/proyecto-wizard.js` y, si es necesario, el punto mínimo autorizado en `app/core/data.js`/backend bridge. Debe:
...
- preservar exactamente el diseño aprobado salvo los controles necesarios para elegir/configurar fuente.

No crear nueva candidata ni reescribir módulos.

## Academia

Actualizar manual de configuración de proyecto con: elección de fuente interna/externa, provider/mapping, autoridad, estado de sincronización, conflicto y diferencia entre Hoja de Ruta y cuestionario.
