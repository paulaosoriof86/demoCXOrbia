...
CX.shopperVisitDetail = function(data, p, v, ui){
  ...
  ui.modal(`${v.sucursal}<div style="font-size:12px;font-weight:500;color:var(--t3);margin-top:2px">📍 ${v.ciudad}, ${v.pais}</div>`, `
    ...
    ${(()=>{const flujo=['publicada','postulada','asignada','agendada','realizada','cuestionario','submitida','validada','liquidada'];const est=(v.estado||'').replace('disponible','publicada').replace('cuestionario','cuestionario');const idx=flujo.indexOf(est);const hist=v.historial||[];return `<div style="border:1px solid var(--border);border-radius:11px;padding:11px 14px;margin-bottom:10px"><div style="font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:var(--t3);margin-bottom:8px">🕐 Historial de estados</div><div style="display:flex;flex-wrap:wrap;gap:4px">${flujo.map((f,i)=>`<span style="font-size:10.5px;padding:2px 8px;border-radius:20px;${i<=idx?'background:var(--brand);color:#fff':'background:var(--panel-2);color:var(--t3)'}">${f}</span>`).join('<span style="color:var(--t3)">›</span>')}</div>${(v.estado==='fuera_rango')?`<div style="margin-top:8px;font-size:11.5px;color:var(--red)">⚠️ Fuera de rango${v.frMotivo?' · '+v.frMotivo:''}${v.frResponsable?' · responsable: '+v.frResponsable:''} ${v.frResponsable&&v.frResponsable!=='shopper'?'(no penaliza al shopper)':''}</div>`:''}${hist.length?'<div style="margin-top:8px;font-size:10.5px;color:var(--t3)">'+hist.map(h=>h.estado+' ('+h.fecha+(h.por?' · '+h.por:'')+')').join(' → ')+'</div>':''}</div>`;})()}
