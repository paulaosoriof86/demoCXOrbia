...
CX.module('movimientos', ({data,ui})=>{
  ...
  const draw=()=>{
    ...
    const aggByCur={}; const groupByCurrency=aggByCur;
    const pendingCurrencyRows=[];
    ...
    <div class="grid g2" style="gap:14px;margin-bottom:16px">
      <div class="card card-p"><div class="card-h"><div class="card-t">Ingresos por tipo</div></div>
        ${(()=>{const porTipoMoneda={}; movs.filter(m=>m.monto>0&&curOf(m)!==PENDING_CURRENCY).forEach(m=>{const t=m.tipoIngreso||'otro';const cu=curOf(m);(porTipoMoneda[cu]=porTipoMoneda[cu]||{})[t]=(porTipoMoneda[cu][t]||0)+m.monto;}); const cus=Object.keys(porTipoMoneda).filter(c=>c!==PENDING_CURRENCY); if(!cus.length) return '<div class="muted" style="font-size:12px;padding:8px 0">Sin ingresos registrados</div>'; return cus.map(cu=>`<div style="font-size:10.5px;font-weight:700;color:var(--t3);margin:6px 0 2px">Moneda ${cu}</div>`+Object.keys(TI).map(t=>{const val=porTipoMoneda[cu][t]||0;return val?`<div class="between" style="padding:6px 0;border-bottom:1px solid var(--border-2)"><span style="font-size:12px;color:var(--t2)">${TI[t]}${t==='financiamiento'?' <span class="bdg bdg-a" style="font-size:9px">→CxP</span>':''}</span><b style="font-size:12.5px;color:${t==='financiamiento'?'var(--amber)':'var(--green)'}">${ui.money(cu,val)}</b></div>`:'';}).join('')).join('');})()}
    ...
    <div class="grid g2" style="gap:14px;margin-bottom:16px">
      <div class="card card-p" id="presMesCard"><div class="between" style="margin-bottom:8px"><div class="card-t">📋 Presupuesto mensual <span class="bdg bdg-b">${per}</span></div><button class="btn btn-soft btn-sm" id="addPresMes">＋ Rubro</button></div><div id="presMesList"></div><div style="font-size:11px;color:var(--t3);margin-top:8px">Es <b>mensual</b> y queda ligado al periodo canónico. El presupuesto es <b>planeado</b>: sin país/moneda asignada se muestra <b>sin moneda</b> y no afecta el margen.</div></div>
      <div class="card card-p" id="finCard"><div class="between" style="margin-bottom:8px"><div class="card-t">🏦 Financiamientos</div><button class="btn btn-soft btn-sm" id="addFin">＋ Financiamiento</button></div><div id="finList"></div><div style="font-size:11px;color:var(--t3);margin-top:8px">No son ingreso operativo: entran como flujo + CxP y se controlan hasta su <b>devolución</b> (egreso).</div></div>
    </div>

    <div class="card card-p"><div class="card-h"><div class="card-t">Movimientos${isG?' globales':' del proyecto'}</div><span class="muted" style="font-size:11px">conceptos categorizados · anti-duplicado al importar</span></div><div class="scroll-hint" aria-label="Desliza para ver más" style="overflow-x:auto"><table class="tbl"><thead><tr><th>Fecha</th><th>Concepto</th><th>Categoría</th><th>Tipo</th><th>País</th><th style="text-align:right">Monto</th><th>Estado</th><th></th></tr></thead><tbody>${movs.map(m=>`<tr><td style="font-size:12px">${m.fecha}</td><td><b>${m.cat}</b><div style="font-size:10px;color:var(--t3)">${m.desc||''}</div></td><td style="font-size:11.5px">${m.categoria||(m.global?'Administrativo':'Proyecto')}</td><td style="font-size:11px">${TI[m.tipoIngreso]||TE[m.tipoEgreso]||m.tipo}</td><td>${m.pais||'—'}</td><td style="text-align:right;font-weight:700;color:var(--${m.monto<0?'red':'green'})">${curOf(m)===PENDING_CURRENCY?'<span class="bdg bdg-a">Pendiente de moneda</span>':(m.monto<0?'− ':'+ ')+_mvMoney(m,Math.abs(m.monto))}</td><td>${ui.bdg(m.estado||'—',(m.estado||'').includes('Cx')?'a':m.monto<0?'r':'g')}</td><td style="text-align:right">${m.id?`<button class="btn btn-ghost btn-sm" data-delm="${m.id}" style="color:var(--red);padding:2px 6px">✕</button>`:''}</td></tr>`).join('')}</tbody></table></div><div style="margin-top:14px">${ui.aiBox('Separo ingresos por comisiones, honorarios, anticipos y facturación de los financiamientos (que van a CxP). Registro CxC/CxP iniciales en la importación y vinculo cada abono a su egreso. Las remesas recibidas se concilian aquí.','Tesorería completa, no solo gastos del proyecto')}</div></div>`;
    ...
    if(mx&&CX.reportKit){
      ...
      const movSpec=(ext)=>({ title:'Movimientos & Tesorería',meta:{title:'Movimientos & Tesorería',project:(isG?'Global (administrativo)':projectLabel),period:canonicalPeriodLabel,scope:(isG?'Global':projectLabel),sourceLabel:'Finanzas · movimientos del periodo',generatedAt:new Date().toLocaleDateString('es-MX',{year:'numeric',month:'long',day:'numeric'})},columns:[{key:'fecha',label:'Fecha'},{key:'concepto',label:'Concepto'},{key:'categoria',label:'Categoría'},{key:'tipo',label:'Tipo'},{key:'pais',label:'País'},{key:'moneda',label:'Moneda'},{key:'monto',label:'Monto'},{key:'estado',label:'Estado'}],rows:exportMovs.map(m=>({fecha:m.fecha||'—',concepto:m.cat||m.desc||'—',categoria:m.categoria||(m.global?'Administrativo':'Proyecto'),tipo:(TI[m.tipoIngreso]||TE[m.tipoEgreso]||m.tipo||'—'),pais:m.pais||'—',moneda:curOf(m),monto:Math.round(m.monto||0),estado:m.estado||'—'})),notes:'Cada fila conserva su moneda; los totales por moneda no se suman entre sí.',summary:['Movimientos: '+exportMovs.length].concat(shownCurs.map(cu=>{const a=aggByCur[cu]||{};return cu+' — Ingresos oper.: '+ui.money(cu,a.ingOper||0)+' · Egresos: '+ui.money(cu,Math.abs(a.egr||0));})),chart:{title:'Ingresos vs Egresos por moneda',perCurrency:chartByCur,data:Object.entries(chartByCur).flatMap(([cu,o])=>Object.entries(o).map(([k2,v2])=>({label:cu+' · '+k2,value:Math.round(v2)})))},reviewSection:pendingCurrencyRows.length?{title:'Filas en revisión (moneda no resuelta)',rows:pendingCurrencyRows.map(x=>({tipo:x.kind,concepto:(x.r.cat||x.r.concepto||x.r.desc||'—'),pais:x.r.pais||'—',monto:Math.round(x.r.monto||x.r.saldo||x.r.total||0),estado:'Pendiente de moneda'}))}:null,filename:[san('movimientos'),san(projectLabel),san(per),new Date().toISOString().slice(0,10)].join('_')+'.'+ext });
    ...
    if(pml){ const store=CX.finStore.pres(p.id,canonicalPeriodId); const ks=Object.keys(store); const tot=ks.reduce((a,k)=>a+(+store[k]||0),0); pml.innerHTML=(ks.length?ks.map(k=>`<div class="between" style="padding:6px 0;border-bottom:1px solid var(--border-2)"><span style="font-size:12.5px">${k}</span><div class="flex" style="gap:8px"><b style="font-size:12.5px">${(+store[k]).toLocaleString()} <span class="muted" style="font-weight:500">sin moneda asignada</span></b><button class="btn btn-ghost btn-sm" data-delpm="${k}" style="color:var(--red);padding:2px 7px">✕</button></div></div>`).join(''):'<div class="muted" style="font-size:12px;padding:6px 0">Sin rubros este mes</div>')+`<div class="between" style="padding:8px 0 0;font-weight:700"><span style="font-size:12.5px">Total planeado</span><b>${tot.toLocaleString()} <span class="muted" style="font-weight:500">pendiente de asignación</span></b></div>`; pml.querySelectorAll('[data-delpm]').forEach(b=>b.addEventListener('click',()=>{CX.finStore.delPres(p.id,b.dataset.delpm,canonicalPeriodId);draw();})); }
    const apm=host.querySelector('#addPresMes'); if(apm)apm.addEventListener('click',()=>ui.modal('Nuevo rubro de presupuesto · '+per,`<label class="lbl">Concepto</label><input class="inp" id="pmK" placeholder="Ej. Coordinación, Transporte" style="margin-bottom:10px"><label class="lbl">Monto planeado</label><input class="inp" id="pmV" type="number" style="margin-bottom:6px"><div style="font-size:11px;color:var(--t3);margin-bottom:14px">Sin moneda asignada hasta definir país/moneda del rubro.</div><div style="text-align:right"><button class="btn btn-pr btn-sm" id="pmSave">Agregar</button></div>`,{onMount:(ov,close)=>{ov.querySelector('#pmSave').addEventListener('click',()=>{const k=(ov.querySelector('#pmK').value||'').trim();if(!k){ui.toast('Escribe el concepto','warn');return;}CX.finStore.setPres(p.id,k,+ov.querySelector('#pmV').value||0,canonicalPeriodId);close();draw();ui.toast('Rubro agregado al presupuesto','ok');});}}));

    const fl=host.querySelector('#finList');
    if(fl){ const fins=CX.finStore.financiamientos(p.id); fl.innerHTML=fins.length?fins.map(f=>`<div style="padding:8px 0;border-bottom:1px solid var(--border-2)"><div class="between"><div><b style="font-size:12.5px">${f.fuente||'Financiamiento'}</b><div style="font-size:10.5px;color:var(--t3)">${f.pais||''} · ${f.fecha} · devuelto ${f.pais&&p.currency[f.pais]?ui.money(p.currency[f.pais],f.devuelto||0):(f.moneda?ui.money(f.moneda,f.devuelto||0):'—')}</div></div><div class="flex" style="gap:8px"><b style="font-size:12.5px;color:${(f.saldo||0)>0?'var(--amber)':'var(--green)'}">saldo ${f.pais&&p.currency[f.pais]?ui.money(p.currency[f.pais],f.saldo||0):(f.moneda?ui.money(f.moneda,f.saldo||0):'Pendiente de moneda')}</b>${(!(f.pais&&p.currency[f.pais])&&!f.moneda)?ui.bdg('Revisión requerida · sin moneda · Bloqueado','r'):((f.saldo||0)<=0?ui.bdg('saldado','g'):`<button class="btn btn-soft btn-sm" data-devfin="${f.id}">Devolver</button>`)}</div></div></div>`).join(''):'<div class="muted" style="font-size:12px;padding:6px 0">Sin financiamientos registrados</div>'; fl.querySelectorAll('[data-devfin]').forEach(b=>b.addEventListener('click',()=>{const f=CX.finStore.financiamientos(p.id).find(x=>x.id===b.dataset.devfin); ui.modal('Devolver financiamiento · '+f.fuente,`<div style="font-size:12.5px;color:var(--t2);margin-bottom:10px">Saldo: <b>${f.pais&&p.currency[f.pais]?ui.money(p.currency[f.pais],f.saldo||0):(f.moneda?ui.money(f.moneda,f.saldo||0):'Pendiente de moneda')}</b></div><label class="lbl">Monto a devolver</label><input class="inp" id="dvM" type="number" value="${f.saldo||0}" style="margin-bottom:14px"><div style="text-align:right"><button class="btn btn-green btn-sm" id="dvOk">Registrar devolución</button></div>`,{onMount:(ov,close)=>{ov.querySelector('#dvOk').addEventListener('click',()=>{CX.finStore.devolverFinanciamiento(p.id,f.id,+ov.querySelector('#dvM').value||0);close();draw();ui.toast('Devolución registrada · egreso generado · CxP reducida','ok',3600);});}}); })); }
    const af=host.querySelector('#addFin'); if(af)af.addEventListener('click',()=>ui.modal('Registrar financiamiento',`<p style="font-size:12px;color:var(--t2);margin-bottom:10px">Entra como <b>flujo</b> (no ingreso operativo) y como CxP hasta devolverse.</p><div class="grid g2" style="gap:10px 12px"><div style="grid-column:1/3"><label class="lbl">Fuente</label><input class="inp" id="fnF" placeholder="Banco / socio / casa matriz"></div><div style="grid-column:1/3"><label class="lbl">Concepto / destino</label><input class="inp" id="fnC" placeholder="Ej. capital de trabajo, anticipo de nómina"></div><div><label class="lbl">Monto <span id="fnCurLbl" class="muted">(elige país)</span></label><input class="inp" id="fnM" type="number"></div><div><label class="lbl">País</label><select class="sel" id="fnP"><option value="">— elige país —</option>${p.countries.map(c=>`<option>${c}</option>`).join('')}</select></div></div><div id="fnReview" style="font-size:11px;color:var(--red);margin-top:6px;display:none">Pendiente de moneda · Bloqueado hasta elegir país</div><div style="text-align:right;margin-top:14px"><button class="btn btn-pr btn-sm" id="fnSave">Registrar</button></div>`,{onMount:(ov,close)=>{const fnCur=()=>{const c=ov.querySelector('#fnP').value;return c&&p.currency[c]?p.currency[c]:PENDING_CURRENCY;};const syncFn=()=>{const cu=fnCur();ov.querySelector('#fnCurLbl').textContent=cu===PENDING_CURRENCY?'(elige país)':'('+cu+')';ov.querySelector('#fnReview').style.display=cu===PENDING_CURRENCY?'':'none';};ov.querySelector('#fnP').addEventListener('change',syncFn);syncFn();ov.querySelector('#fnSave').addEventListener('click',()=>{if(fnCur()===PENDING_CURRENCY){ui.toast('Selecciona el país para resolver la moneda antes de registrar','warn');return;}CX.finStore.addFinanciamiento(p.id,{fuente:(ov.querySelector('#fnF').value||'').trim(),concepto:(ov.querySelector('#fnC').value||'').trim(),monto:+ov.querySelector('#fnM').value||0,pais:ov.querySelector('#fnP').value,moneda:fnCur()});close();draw();ui.toast('Financiamiento registrado · flujo + CxP (no operativo)','ok',3600);});}}));
    const cxpF=host.querySelector('#cxpFind'); if(cxpF)cxpF.addEventListener('input',()=>{const q=cxpF.value.toLowerCase();host.querySelectorAll('#cxpBody .cxpRow').forEach(r=>{r.style.display=r.textContent.toLowerCase().includes(q)?'':'none';});});
    ...
    host.querySelectorAll('[data-cxdet]').forEach(el=>el.addEventListener('click',()=>{ const [kind,id]=el.dataset.cxdet.split(':'); const arr=kind==='cxc'?CX.finStore.cxc(pid()):CX.finStore.cxp(pid()); const r=arr.find(x=>x.id===id); if(!r)return; const estados=kind==='cxc'?['pendiente','parcial','cobrada','incobrable']:['pendiente','parcial','pagada','programada']; ui.modal((kind==='cxc'?'⏳ Cuenta por cobrar':'💸 Cuenta por pagar')+' · '+r.concepto,`<div style="font-size:12.5px;line-height:1.9;color:var(--t2);margin-bottom:12px"><div><b>Concepto:</b> ${r.concepto}</div><div><b>País:</b> ${r.pais||'—'} · <b>Origen:</b> ${r.origen||'manual'}</div>${r.shopper?`<div><b>Shopper/Acreedor:</b> ${r.shopper}</div>`:''}${r.visitaId?`<div><b>Visita vinculada:</b> ${r.visitaId}</div>`:''}</div><div class="grid g2" style="gap:10px 12px"><div><label class="lbl">País</label><select class="sel" id="cxPais"><option value="">—</option>${p.countries.map(c=>`<option ${r.pais===c?'selected':''}>${c}</option>`).join('')}</select></div><div><label class="lbl">Monto <span id="cxCurLbl" class="muted">(elige país)</span></label><input class="inp" id="cxSaldo" type="number" value="${r.saldo||0}"></div><div><label class="lbl">Estado</label><select class="sel" id="cxEst">${estados.map(s=>`<option ${(r.estado||'pendiente')===s?'selected':''}>${s}</option>`).join('')}</select></div><div style="grid-column:1/3"><label class="lbl">Nota</label><input class="inp" id="cxNota" value="${(r.nota||'').replace(/"/g,'&quot;')}" placeholder="Observación"></div></div><div id="cxReview" style="font-size:11px;color:var(--red);margin-top:6px;display:none">Pendiente de moneda · Bloqueado: elige país para resolver la moneda antes de guardar el saldo</div><div class="between" style="margin-top:14px"><button class="btn btn-ghost btn-sm" id="cxDel" style="color:var(--red)">🗑 Eliminar</button><button class="btn btn-pr btn-sm" id="cxSave">Guardar cambios</button></div>`,{onMount:(ov,close)=>{const cxCur=()=>{const c=ov.querySelector('#cxPais').value;return c&&p.currency[c]?p.currency[c]:(r.moneda||PENDING_CURRENCY);};const syncCx=()=>{const cu=cxCur();ov.querySelector('#cxCurLbl').textContent=cu===PENDING_CURRENCY?'(elige país)':'('+cu+')';ov.querySelector('#cxReview').style.display=cu===PENDING_CURRENCY?'':'none';};ov.querySelector('#cxPais').addEventListener('change',syncCx);syncCx();ov.querySelector('#cxSave').addEventListener('click',()=>{if(cxCur()===PENDING_CURRENCY){ui.toast('Elige país para resolver la moneda antes de guardar el saldo','warn');return;}CX.finStore.editCx(pid(),kind,id,{saldo:+ov.querySelector('#cxSaldo').value||0,estado:ov.querySelector('#cxEst').value,nota:ov.querySelector('#cxNota').value,pais:ov.querySelector('#cxPais').value,moneda:cxCur()});close();draw();ui.toast('Cuenta actualizada','ok');});ov.querySelector('#cxDel').addEventListener('click',()=>{CX.finStore.delCx(pid(),kind,id);close();draw();ui.toast('Cuenta eliminada','');});}}); }));

    host.querySelectorAll('[data-drill]').forEach(el=>el.addEventListener('click',()=>{ const k=el.dataset.drill; let title,rows; if(k==='ing'||k==='egr'){const f=movs.filter(m=>(k==='ing'?m.monto>0:m.monto<0)&&curOf(m)!==PENDING_CURRENCY);title=k==='ing'?'Ingresos':'Egresos';rows=f.map(m=>`<tr><td>${m.fecha}</td><td><b>${m.cat}</b></td><td>${TI[m.tipoIngreso]||TE[m.tipoEgreso]||m.tipo}</td><td>${curOf(m)}</td><td style="text-align:right">${_mvMoney(m,Math.abs(m.monto))}</td></tr>`).join('');} else if(k==='rem'){const f=movs.filter(m=>m.tipoIngreso==='remesa'&&curOf(m)!==PENDING_CURRENCY);title='Remesas recibidas';rows=f.map(m=>`<tr><td>${m.fecha}</td><td><b>${m.cat}</b></td><td>${m.estado||''}</td><td>${curOf(m)}</td><td style="text-align:right">${_mvMoney(m,m.monto)}</td></tr>`).join('');} else {const arr=k==='cxc'?CX.finStore.cxc(pid()):CX.finStore.cxp(pid());title=k==='cxc'?'Cuentas por cobrar':'Cuentas por pagar';rows=arr.map(r=>`<tr><td><b>${r.concepto}</b></td><td>${r.pais||'Pendiente de moneda'}</td><td style="text-align:right">${r.pais?ui.money(curOfRow(r),r.saldo||0):'Pendiente de moneda'}</td></tr>`).join('');} ui.modal(title,rows?`<table class="tbl"><tbody>${rows}</tbody></table>`:ui.empty('💰','Sin registros.')); }));

    host.querySelectorAll('[data-new]').forEach(b=>b.addEventListener('click',()=>{ const t=b.dataset.new; const esIng=t!=='egreso'; const tipos=esIng?TI:TE; const defTipo=t==='remesa'?'remesa':Object.keys(tipos)[0]; ui.modal('Registrar '+(t==='remesa'?'remesa recibida':t),`<div class="grid g2" style="gap:10px 12px"><div style="grid-column:1/3"><label class="lbl">Concepto</label><input class="inp" id="mvCat" placeholder="Concepto del movimiento"></div><div><label class="lbl">Categoría</label><select class="sel" id="mvCateg">${CAT.map(c=>`<option ${(scope==='global'&&c==='Administrativo')?'selected':''}>${c}</option>`).join('')}</select></div><div><label class="lbl">Tipo de ${esIng?'ingreso':'egreso'}</label><select class="sel" id="mvTipo">${Object.keys(tipos).map(k=>`<option value="${k}" ${k===defTipo?'selected':''}>${tipos[k]}</option>`).join('')}</select></div><div><label class="lbl">Monto <span id="mvCurLbl" class="muted">(elige país)</span></label><input class="inp" id="mvMonto" type="number"></div><div><label class="lbl">País</label><select class="sel" id="mvPais"><option value="">— elige país —</option>${p.countries.map(c=>`<option ${scope!=='global'?'selected':''}>${c}</option>`).join('')}</select></div><div><label class="lbl">Fecha (admite histórico)</label><input class="inp" id="mvFecha" type="date" value="${new Date().toISOString().slice(0,10)}"></div><div><label class="lbl">Estado</label><select class="sel" id="mvEstado">${(esIng?['Conciliado','Pendiente (CxC)','Por conciliar']:['Pagado','Programado']).map(s=>`<option>${s}</option>`).join('')}</select></div><div><label class="lbl">${esIng?'Pagador / fuente':'Beneficiario'}</label><input class="inp" id="mvBenef" placeholder="${esIng?'Quién paga (cliente/casa matriz)':'A quién se paga (shopper/proveedor)'}"></div><div><label class="lbl">Proyecto destino</label><select class="sel" id="mvProy"><option value="">— sin proyecto —</option>${(data.projects||[]).map(pr=>`<option value="${pr.id}" ${pr.id===pid()?'selected':''}>${pr.name}</option>`).join('')}</select></div><div style="grid-column:1/3"><label class="lbl">Descripción</label><input class="inp" id="mvDesc" placeholder="Opcional"></div></div><div style="text-align:right;margin-top:14px"><button class="btn btn-pr btn-sm" id="mvSave">Registrar</button></div>`,{onMount:(ov,close)=>{const mvCur=()=>{const c=ov.querySelector('#mvPais').value;return c&&p.currency[c]?p.currency[c]:PENDING_CURRENCY;};const syncMv=()=>{const cu=mvCur();ov.querySelector('#mvCurLbl').textContent=cu===PENDING_CURRENCY?'(elige país)':'('+cu+')';};ov.querySelector('#mvPais').addEventListener('change',syncMv);syncMv();ov.querySelector('#mvSave').addEventListener('click',()=>{if(mvCur()===PENDING_CURRENCY){ui.toast('Selecciona el país para resolver la moneda antes de registrar','warn');return;}const monto=Math.abs(+ov.querySelector('#mvMonto').value||0)*(esIng?1:-1);const rec={tipo:esIng?'ingreso':'egreso',cat:ov.querySelector('#mvCat').value||t,categoria:ov.querySelector('#mvCateg').value,pais:ov.querySelector('#mvPais').value,moneda:mvCur(),monto,fecha:ov.querySelector('#mvFecha').value,desc:ov.querySelector('#mvDesc').value,estado:ov.querySelector('#mvEstado').value,beneficiario:(ov.querySelector('#mvBenef').value||'').trim(),proyectoId:ov.querySelector('#mvProy').value};if(esIng)rec.tipoIngreso=ov.querySelector('#mvTipo').value;else rec.tipoEgreso=ov.querySelector('#mvTipo').value;CX.finStore.addMov(pid(),rec);close();draw();ui.toast('Movimiento registrado','ok');});}}); }));

    const acx=host.querySelector('#autoCxp'); if(acx)acx.addEventListener('click',()=>{ const liqPend=CX.liq.forProject(data).filter(l=>['validada','pendiente_submitir','conciliada_pendiente_pago'].includes(l.estado)&&!isFinancialReview(l)); const yaCxp=new Set(CX.finStore.cxp(pid()).map(r=>r.visitaId).filter(Boolean)); const nuevasCxp=liqPend.filter(l=>!yaCxp.has(l.visitaId)); const cxcEst=[]; ui.modal('⚙️ Generar CxC/CxP automáticas',`<p style="font-size:12.5px;color:var(--t2);margin-bottom:12px">Deriva cuentas automáticamente del histórico operativo. Revisa y confirma:</p><div class="card-t" style="font-size:12.5px;margin-bottom:6px">📤 Cuentas por pagar (liquidaciones pendientes no pagadas)</div>${nuevasCxp.length?`<table class="tbl" style="margin-bottom:12px"><tbody>${nuevasCxp.slice(0,8).map(l=>`<tr><td><b>${l.shopper}</b><div style="font-size:10px;color:var(--t3)">${l.sucursal}</div></td><td style="text-align:right;font-weight:700">${currencyOf(l)!==PENDING_CURRENCY?ui.money(currencyOf(l),l.total):'<span class="bdg bdg-a" style="font-size:9px">Pendiente de moneda</span>'}</td></tr>`).join('')}${nuevasCxp.length>8?`<tr><td colspan="2" style="font-size:11px;color:var(--t3);text-align:center">+${nuevasCxp.length-8} más</td></tr>`:''}</tbody></table>`:'<div class="muted" style="font-size:12px;margin-bottom:12px">Sin liquidaciones pendientes nuevas.</div>'}<div class="card-t" style="font-size:12.5px;margin-bottom:6px">📥 Cuentas por cobrar (reembolsos no conciliados)</div>${cxcEst.length?`<table class="tbl" style="margin-bottom:12px"><tbody>${cxcEst.map(x=>`<tr><td><b>Reembolso pendiente · ${CX.paisLabel(x.c)}</b></td><td style="text-align:right;font-weight:700">${x.cur} ${x.monto.toLocaleString()}</td></tr>`).join('')}</tbody></table>`:'<div class="muted" style="font-size:12px;margin-bottom:12px">Reembolsos conciliados.</div>'}<div style="background:var(--brand-light);border-radius:9px;padding:9px 12px;font-size:11.5px;color:var(--brand-dark);margin-bottom:12px">Las CxP por liquidación se cruzan automáticamente con el egreso cuando se pagan; las CxC se descargan al conciliar el reembolso.</div><div style="text-align:right"><button class="btn btn-green btn-sm" id="acxOk">Generar ${nuevasCxp.length+cxcEst.length} cuenta(s)</button></div>`,{onMount:(ov,close)=>{ov.querySelector('#acxOk').addEventListener('click',()=>{nuevasCxp.forEach(l=>CX.finStore.addCxp(pid(),{concepto:'Liquidación pendiente · '+l.shopper+' ('+l.sucursal+')',monto:l.total,pais:l.pais,origen:'liquidacion',visitaId:l.visitaId,auto:true}));cxcEst.forEach(x=>CX.finStore.addCxc(pid(),{concepto:'Reembolso pendiente de conciliar · '+CX.paisLabel(x.c),monto:x.monto,pais:x.c,origen:'reembolso',auto:true}));close();draw();ui.toast((nuevasCxp.length+cxcEst.length)+' cuenta(s) generadas automáticamente del histórico','ok',4000);});}}); });

    host.querySelectorAll('[data-cuenta]').forEach(b=>b.addEventListener('click',()=>{ const k=b.dataset.cuenta; ui.modal('Registrar cuenta por '+(k==='cxc'?'cobrar':'pagar'),`<p style="font-size:12px;color:var(--t2);margin-bottom:10px">Útil para cargar saldos iniciales en la importación o registrar deudas/derechos del periodo.</p><div class="grid g2" style="gap:10px 12px"><div style="grid-column:1/3"><label class="lbl">Concepto / contraparte</label><input class="inp" id="ctCon" list="ctConList" placeholder="${k==='cxc'?'Cliente / casa matriz':'Proveedor / financiamiento'}"></div><datalist id="ctConList">${(k==='cxc'?CX.finStore.cxc(pid()):CX.finStore.cxp(pid())).map(r=>`<option value="${r.concepto}">`).join('')}${CX.data._visitas.filter(v=>v.projectId===CX.data.currentPeriodId).map(v=>v.shopper).filter((s,i,a)=>s&&a.indexOf(s)===i).map(s=>`<option value="${s}">`).join('')}</datalist><div><label class="lbl">País</label><select class="sel" id="ctPais"><option value="">—</option>${p.countries.map(c=>`<option>${c}</option>`).join('')}</select></div><div><label class="lbl">Monto <span id="ctCurLbl" class="muted">(elige país)</span></label><input class="inp" id="ctMonto" type="number"></div><div style="grid-column:1/3"><label class="lbl">Vence</label><input class="inp" id="ctVence" type="date"></div></div><div style="text-align:right;margin-top:14px"><button class="btn btn-pr btn-sm" id="ctSave">Registrar</button></div>`,{onMount:(ov,close)=>{const ctCur=()=>{const c=ov.querySelector('#ctPais').value;return c&&p.currency[c]?p.currency[c]:PENDING_CURRENCY;};const syncCt=()=>{const cu=ctCur();ov.querySelector('#ctCurLbl').textContent=cu===PENDING_CURRENCY?'(elige país)':'('+cu+')';};ov.querySelector('#ctPais').addEventListener('change',syncCt);syncCt();ov.querySelector('#ctSave').addEventListener('click',()=>{if(ctCur()===PENDING_CURRENCY){ui.toast('Selecciona el país para resolver la moneda antes de registrar','warn');return;}const r={concepto:ov.querySelector('#ctCon').value||'(sin concepto)',monto:+ov.querySelector('#ctMonto').value||0,pais:ov.querySelector('#ctPais').value,moneda:ctCur(),vence:ov.querySelector('#ctVence').value};if(k==='cxc')CX.finStore.addCxc(pid(),r);else CX.finStore.addCxp(pid(),r);close();draw();ui.toast('Cuenta por '+(k==='cxc'?'cobrar':'pagar')+' registrada','ok');});}}); }));

    host.querySelectorAll('[data-abono]').forEach(b=>b.addEventListener('click',()=>{ const r=CX.finStore.cxp(pid()).find(x=>x.id===b.dataset.abono); if(!r||currencyOf(r)===PENDING_CURRENCY){ui.toast('Cuenta sin moneda resuelta: resuélvela en “ver detalle” antes de abonar','warn');return;} ui.modal('Abonar a CxP · '+r.concepto,`<div style="font-size:12.5px;color:var(--t2);margin-bottom:10px">Saldo actual: <b>${r.pais?ui.money(curOfRow(r),r.saldo||0):'Pendiente de moneda'}</b></div><label class="lbl">Monto del abono (${curOfRow(r)})</label><input class="inp" id="abMonto" type="number" value="${r.saldo||0}" style="margin-bottom:14px"><div style="text-align:right"><button class="btn btn-green btn-sm" id="abSave">Registrar abono</button></div>`,{onMount:(ov,close)=>{ov.querySelector('#abSave').addEventListener('click',()=>{CX.finStore.abonarCxp(pid(),r.id,+ov.querySelector('#abMonto').value||0);close();draw();ui.toast('Abono registrado · egreso vinculado','ok');});}}); }));
    ...
    pl.addEventListener('click',async()=>{
      if(pendingCurrencyRows.length){ui.toast('Pagar lote bloqueado: resuelve las filas en revisión de moneda primero','err');return;}
      ...
      try{
        ...
        try{if(CX.backend?.refresh)await CX.backend.refresh();}catch(_){/* committed; refresh best-effort */}
    ...
    const ih=host.querySelector('#impHist'); if(ih)ih.addEventListener('click',()=>ui.modal('Importar histórico de movimientos',`<p style="font-size:12.5px;color:var(--t2);margin-bottom:12px">Sube tu archivo (Excel/CSV) de movimientos, remesas, CxC/CxP. Vista previa + anti-duplicado por fecha+monto+concepto.</p><input type="file" class="inp" style="padding:7px;margin-bottom:12px"><div style="background:var(--brand-light);border-radius:9px;padding:10px 12px;font-size:12px;color:var(--brand-dark)">Mapeo de columnas → tipo/categoría/monto/fecha/país/estado. Permite cargar <b>saldos iniciales</b> de CxC/CxP y remesas para conciliar.</div><div style="text-align:right;margin-top:14px"><button class="btn btn-pr btn-sm" onclick="CX.ui.toast('Vista previa lista (demo)','ok');this.closest('.cx-ov').remove()">Ver vista previa</button></div>`));
  };
  draw();
  CX.bus.on('fin',()=>draw());
  return host;
...
CX.module('liquidaciones', ({data,ui})=>{
  ...
  const isFinancialReview=(l)=>{const cur=l&&(l.moneda||(l.pais&&p.currency&&p.currency[l.pais]));return !l||l.reviewRequired===true||l.financialSourceStatus==='pending_or_review'||l.liquidationState==='pending_financial_source'||!l.pais||!cur;};
  const host=ui.el('div');
  const draw=()=>{
    ...
    CX.finStore._draft[p.id]=draft;
    const oblig=p.countries.map(c=>{const ls=all.filter(l=>l.pais===c);const hon=ls.reduce((a,l)=>a+l.honorario,0), reemb=ls.reduce((a,l)=>a+l.reembolso,0), tot=ls.reduce((a,l)=>a+l.total,0);const listo=ls.filter(l=>l.estado==='validada').reduce((a,l)=>a+l.total,0);return `<tr><td><b>${c}</b></td><td>${p.currency[c]}</td><td>${ls.length}</td><td>${ui.money(p.currency[c],hon)}</td><td>${ui.money(p.currency[c],reemb)}</td><td><b>${ui.money(p.currency[c],tot)}</b></td><td>${ui.money(p.currency[c],listo)}</td></tr>`;}).join('');
    const lrow=(l,i)=>{const lb=CX.liq.label(l.estado); const inD=draft.includes(l.visitaId);return `<tr data-li="${i}" style="${inD?'background:var(--brand-light)':''}"><td style="position:sticky;left:0;background:${inD?'#eaf4fc':'var(--panel)'};z-index:1">${l.estado==='validada'?(inD?`<button class="btn btn-soft btn-sm" data-rm="${l.visitaId}" style="padding:3px 9px;color:var(--red)">✕ Retirar</button>`:`<button class="btn btn-pr btn-sm" data-add="${l.visitaId}" style="padding:3px 10px">▶ Mover a lote</button>`):l.estado==='pendiente_cuestionario'?ui.bdg('espera shopper','n'):l.estado==='pagada'?ui.bdg('✓ pagada','g'):l.estado==='pagada_preview'?ui.bdg('◐ pagada (preview)','a'):ui.bdg('—','n')}</td><td style="position:sticky;left:96px;background:${inD?'#eaf4fc':'var(--panel)'};z-index:1"><b>${l.shopper||'—'}</b><div style="font-size:10px;color:var(--t3)">${l.shopperCode||''}</div></td><td style="font-size:12px">${l.sucursal}</td><td style="font-size:12px">${l.freal||'—'}</td><td>${inD?ui.bdg('● en lote','p'):ui.bdg(lb[0],lb[1])}</td><td>${l.submit?'✅':'—'}</td><td>${_liqMoney(l,l.honorario)}</td><td>${l.reembolso?_liqMoney(l,l.reembolso):'—'}</td><td style="font-weight:700;color:var(--t1)">${_liqMoney(l,l.total)}</td><td style="font-size:12px">${l.fechaEstimadaPago||'—'}</td><td style="text-align:right"><button class="btn btn-ghost btn-sm" data-ledit="${l.visitaId}" title="Editar liquidación" style="padding:2px 7px">✎</button></td></tr>`;};
    ...
    const cart=`<div class="card card-p" style="margin-bottom:16px;border:1px solid ${draft.length?'var(--brand)':'var(--border)'};${draft.length?'background:linear-gradient(180deg,var(--brand-light),var(--surface))':''}"><div class="between" style="margin-bottom:10px"><div class="card-t">📦 Lote en construcción ${draft.length?`<span class="bdg bdg-b">${draft.length}</span>`:''}</div><div class="flex" style="gap:8px">${CX.finStore.cxp(p.id).filter(r=>r.origen==='liquidacion').length?`<button class="btn btn-soft btn-sm" id="addCxp">➕ Incluir CxP meses anteriores (${CX.finStore.cxp(p.id).filter(r=>r.origen==='liquidacion').length})</button>`:''}${draft.length?`<button class="btn btn-ghost btn-sm" id="clearDraft" style="color:var(--red)">Vaciar</button>`:''}</div></div>${draft.length?`<div class="scroll-hint" style="overflow-x:auto"><table class="tbl"><thead><tr><th>Shopper</th><th>Sucursal</th><th>País</th><th style="text-align:right">Total</th><th></th></tr></thead><tbody>${draftLiqs.map(l=>`<tr><td><b>${l.shopper}</b></td><td style="font-size:12px">${l.sucursal}</td><td>${l.pais||'—'}</td><td style="text-align:right;font-weight:700">${_liqMoney(l,l.total)}</td><td style="text-align:right"><button class="btn btn-ghost btn-sm" data-rm="${l.visitaId}" style="color:var(--red);padding:2px 7px">✕</button></td></tr>`).join('')}</tbody></table></div><div class="between" style="margin-top:12px;padding-top:11px;border-top:1px solid var(--border-2)"><div>${Object.keys(porMon).map(m=>`<span style="font-family:var(--disp);font-size:17px;font-weight:800;color:var(--green);margin-right:14px">${ui.money(m,porMon[m])}</span>`).join('')}${draftPending?'<div style="font-size:11px;color:var(--red);margin-top:3px">⚠ '+draftPending+' liquidación(es) sin moneda resuelta · pago bloqueado hasta revisarlas</div>':''}${multiMon?'<div style="font-size:11px;color:var(--red);margin-top:3px">⚠ Hay más de una moneda. Un lote debe ser de una sola moneda; retira las de otra moneda antes de pagar.</div>':''}</div><button class="btn btn-green btn-sm" id="payDraft" ${multiMon||draftPending?'disabled':''}>💳 Pagar lote (${draft.length})</button></div>`:`<div class="muted" style="font-size:12.5px;padding:6px 0">Aún no has movido liquidaciones al lote. Usa <b>▶ Mover a lote</b> en cada fila validada; aquí verás el total a pagar y podrás retirar.</div>`}</div>`;

    host.innerHTML=`<div class="between" style="margin-bottom:12px"><div>${ui.ph('Liquidaciones', p.name+' · sincronizadas con el avance de cada visita')}</div><div class="flex"><span class="bdg bdg-a">◐ Candidatas · pend. cruce</span><button class="btn btn-ghost btn-sm" id="liqExport">⤓ Exportar</button></div></div><div class="card card-p" style="margin-bottom:12px;border-left:4px solid var(--amber);background:var(--amber-bg,#fffbeb)"><div style="font-size:11.5px;color:#92400e;line-height:1.6">📌 Las liquidaciones son <b>candidatas</b> derivadas del avance operativo. El <b>monto final a pagar</b> se confirma al cruzar con el Excel financiero externo del cierre. No se muestra “deuda final” ni “pagos listos” solo desde la HR.</div></div><div class="grid" style="grid-template-columns:repeat(4,1fr);gap:11px;margin-bottom:16px" id="liqKpis"><div data-lk="review" style="cursor:pointer">${ui.kpi('En revisión financiera',reviewLiqAll.length,'r')}</div><div data-lk="reconciled" style="cursor:pointer">${ui.kpi('Conciliadas · pago pendiente',reconciledPendingPayment.length,'a')}</div><div data-lk="lote" style="cursor:pointer">${ui.kpi('Candidatas para lote',loteCandidates.length,'b')}</div><div data-lk="paid" style="cursor:pointer">${ui.kpi('Pagadas confirmadas',paidConfirmed.length,'g')}</div></div>${cart}<div class="card card-p" style="margin-bottom:16px"><div class="card-t" style="margin-bottom:8px">📊 Obligaciones por país y moneda</div><div style="background:var(--amber-bg);border-radius:9px;padding:9px 12px;font-size:11.5px;color:#8a5b00;margin-bottom:12px">No se suman monedas distintas. Total = honorario + reembolso (boleto + combo). El estado y la <b>fecha estimada de pago</b> se derivan del avance de la visita.</div><div class="scroll-hint" style="overflow-x:auto"><table class="tbl"><thead><tr><th>País</th><th>Moneda</th><th>Visitas</th><th>Honorarios</th><th>Reembolsos</th><th>Total</th><th>Listo para lote</th></tr></thead><tbody>${oblig}</tbody></table></div></div><div class="card card-p"><div class="between" style="margin-bottom:12px"><div><div class="card-t">💸 Liquidaciones operativas</div><div style="font-size:11px;color:var(--t3)">El estado avanza solo con la visita. Mueve las validadas al lote y págalas arriba.</div></div></div><div class="scroll-hint" style="overflow-x:auto"><table class="tbl" style="min-width:800px"><thead><tr><th style="position:sticky;left:0;background:var(--panel);z-index:2"></th><th style="position:sticky;left:96px;background:var(--panel);z-index:2">Shopper</th><th>Sucursal</th><th>Realizada</th><th>Estado</th><th>Submit.</th><th>Honorario</th><th>Reembolso</th><th>Total</th><th>Pago est.</th><th></th></tr></thead><tbody>${all.map(lrow).join('')}</tbody></table></div><div style="margin-top:14px">${ui.aiBox('Cada liquidación nace del avance de la visita: realizada → pend. cuestionario → validada → en lote → pagada. Mueve al lote, revisa el total a pagar, retira lo que no entra (queda como CxP del mes) y paga: se generan los egresos automáticamente.','Liquidación sincronizada')}</div></div>`;
    ...
    if(lx&&CX.reportKit){const san=(s)=>String(s||'r').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]+/g,'-').replace(/^-+|-+$/g,'').toLowerCase()||'r';const projectLabel=data.programBase?data.programBase(p):p.name;const _lcur=(l)=>l.moneda||(l.pais&&p.currency&&p.currency[l.pais])||null;const exportable=all.filter(l=>!isFinancialReview(l));const reviewLiq=all.filter(isFinancialReview);const byEst={};exportable.forEach(l=>{byEst[l.estado]=(byEst[l.estado]||0)+1;});const liqSpec=(ext)=>({title:'Liquidaciones',meta:{title:'Liquidaciones',project:projectLabel,period:(p.periodo||p.ronda||p.name||'Periodo'),scope:'Candidatas · pend. cruce',sourceLabel:'Finanzas · liquidaciones derivadas del avance',generatedAt:new Date().toLocaleDateString('es-MX',{year:'numeric',month:'long',day:'numeric'})},columns:[{key:'shopper',label:'Shopper'},{key:'sucursal',label:'Sucursal'},{key:'pais',label:'País'},{key:'moneda',label:'Moneda'},{key:'estado',label:'Estado'},{key:'honorario',label:'Honorario'},{key:'reembolso',label:'Reembolso'},{key:'total',label:'Total'},{key:'pago',label:'Pago est.'}],rows:exportable.map(l=>({shopper:l.shopper||'—',sucursal:l.sucursal||'—',pais:l.pais||'—',moneda:_lcur(l),estado:l.estado,honorario:Math.round(l.honorario||0),reembolso:Math.round(l.reembolso||0),total:Math.round(l.total||0),pago:l.fechaEstimadaPago||'—'})),notes:reviewLiq.length?(reviewLiq.length+' liquidación(es) en revisión de fuente/moneda excluidas del export monetario.'):'',reviewSection:reviewLiq.length?{title:'Liquidaciones en revisión (excluidas)',rows:reviewLiq.map(l=>({shopper:l.shopper||'—',sucursal:l.sucursal||'—',pais:l.pais||'—',estado:'Pendiente de moneda/fuente'}))}:null,summary:['Liquidaciones exportables: '+exportable.length+(reviewLiq.length?(' · en revisión: '+reviewLiq.length):''),Object.entries(byEst).map(([k2,v2])=>k2+': '+v2).join(' · ')],chart:{title:'Liquidaciones por estado',data:Object.entries(byEst).map(([k2,v2])=>({label:k2,value:v2}))},filename:[san('liquidaciones'),san(projectLabel),new Date().toISOString().slice(0,10)].join('_')+'.'+ext});lx.addEventListener('click',()=>{if(!exportable.length){ui.toast('No hay liquidaciones con moneda/fuente resuelta para exportar'+(reviewLiq.length?(' · '+reviewLiq.length+' en revisión'):''),'err',4200);return;}CX.reportKit.openReport(liqSpec('pdf'),'fin_liquidaciones');});}
    ...
    host.querySelectorAll('#liqKpis [data-lk]').forEach(el=>el.addEventListener('click',()=>{const m=lkMap[el.dataset.lk];const arr=all.filter(m[1]);ui.modal(m[0]+' ('+arr.length+')',arr.length?`<table class="tbl"><thead><tr><th>Shopper</th><th>Sucursal</th><th>Total</th><th>Pago est.</th></tr></thead><tbody>${arr.map(l=>`<tr><td><b>${l.shopper||'—'}</b></td><td style="font-size:12px">${l.sucursal}</td><td style="font-weight:700">${_liqMoney(l,l.total)}</td><td style="font-size:12px">${l.fechaEstimadaPago||'—'}</td></tr>`).join('')}</tbody></table>`:ui.empty('💸','Sin liquidaciones en esta categoría.'));}));
    host.querySelectorAll('[data-ledit]').forEach(b=>b.addEventListener('click',()=>{const l=all.find(x=>x.visitaId===b.dataset.ledit);const v=data._visitas.find(x=>x.id===b.dataset.ledit);if(!l||!v)return;ui.modal('Editar liquidación · '+(l.shopper||''),`<div style="font-size:12px;color:var(--t2);margin-bottom:10px">📍 ${l.sucursal} · estado: ${l.estado}</div><div class="grid g2" style="gap:10px 12px"><div><label class="lbl">País</label><select class="sel" id="le_pais"><option value="">—</option>${p.countries.map(c=>`<option ${l.pais===c?'selected':''}>${c}</option>`).join('')}</select></div><div><label class="lbl">Moneda</label><input class="inp" id="le_moneda" value="${l.moneda||(l.pais&&p.currency&&p.currency[l.pais])||''}" placeholder="resuelve país" readonly></div><div><label class="lbl">Honorario <span id="le_curL" class="muted">${_lcur(l)||'(elige país)'}</span></label><input class="inp" id="le_hon" type="number" value="${l.honorario||0}"></div><div><label class="lbl">Reembolso</label><input class="inp" id="le_re" type="number" value="${l.reembolso||0}"></div><div><label class="lbl">Fecha realizada</label><input class="inp" id="le_f" type="date" value="${v.realizada||''}"></div><div><label class="lbl">Estado</label><select class="sel" id="le_est">${['realizada','cuestionario','liquidada'].map(o=>`<option ${o===v.estado?'selected':''}>${o}</option>`).join('')}</select></div></div><div style="background:var(--amber-bg);border-radius:9px;padding:8px 11px;font-size:11px;color:#8a5b00;margin-top:12px">Corrige aquí errores de captura. El cambio se refleja en la liquidación y se sincroniza con Beneficios y Finanzas.</div><div style="text-align:right;margin-top:14px"><button class="btn btn-pr btn-sm" id="le_ok">Guardar corrección</button></div>`,{onMount:(ov,close)=>{const syncLe=()=>{const c=ov.querySelector('#le_pais').value;const cu=c&&p.currency&&p.currency[c]?p.currency[c]:'';ov.querySelector('#le_moneda').value=cu;ov.querySelector('#le_curL').textContent=cu||'(elige país)';};ov.querySelector('#le_pais').addEventListener('change',syncLe);ov.querySelector('#le_ok').addEventListener('click',()=>{const c=ov.querySelector('#le_pais').value;if(!(c&&p.currency&&p.currency[c])){ui.toast('Resuelve el país/moneda antes de guardar montos','warn');return;}v.pais=c;v.moneda=p.currency[c];l.pais=c;l.moneda=p.currency[c];v.honorario=+ov.querySelector('#le_hon').value||0;const re=+ov.querySelector('#le_re').value||0;v.boleto=re;v.comboAmt=0;v.realizada=ov.querySelector('#le_f').value||v.realizada;v.estado=ov.querySelector('#le_est').value;CX.bus&&CX.bus.emit('visit-flow');close();draw();ui.toast('Liquidación corregida · sincronizada','ok',3200);});}});}));
    const cd=host.querySelector('#clearDraft'); if(cd)cd.addEventListener('click',()=>CX.finStore.clearDraft(p.id));
    const ac=host.querySelector('#addCxp'); if(ac)ac.addEventListener('click',()=>{const _cxCur=r=>currencyOf(r);const cxps=CX.finStore.cxp(p.id).filter(r=>currencyOf(r)!==PENDING_CURRENCY&&r.origen==='liquidacion'&&(r.saldo||0)>0);const cxpsPend=CX.finStore.cxp(p.id).filter(r=>r.origen==='liquidacion'&&(r.saldo||0)>0&&currencyOf(r)===PENDING_CURRENCY);const rows=cxps.length?cxps.map((r,i)=>`<label class="between" style="padding:9px 11px;border:1px solid var(--border);border-radius:10px;margin-bottom:8px;cursor:pointer"><span><input type="checkbox" class="cxpChk" data-id="${r.id}" checked style="margin-right:8px"><b style="font-size:12.5px">${r.concepto}</b><div style="font-size:11px;color:var(--t3)">${r.pais||''} · pendiente de meses anteriores</div></span><b style="color:var(--amber)">${(r.pais&&p.currency&&p.currency[r.pais])?ui.money(p.currency[r.pais],r.saldo||0):(r.moneda?ui.money(r.moneda,r.saldo||0):'Pendiente de moneda')}</b></label>`).join(''):ui.empty('📭','No hay liquidaciones diferidas (CxP) pendientes.');ui.modal('Incluir CxP de meses anteriores',`<p style="font-size:12.5px;color:var(--t2);margin-bottom:12px">Estas son liquidaciones <b>diferidas</b> en cierres anteriores. Selecciona cuáles pagar ahora: se generará el egreso y se saldará la cuenta por pagar.</p>${rows}${cxpsPend.length?`<div style="font-size:11px;color:var(--red);margin:6px 0">🔒 ${cxpsPend.length} CxP con Pendiente de moneda · en revisión, no pagables hasta asignar país/moneda</div>`:''}<div style="text-align:right;margin-top:8px"><button class="btn btn-green btn-sm" id="payCxp" ${cxps.length?'':'disabled'}>Pagar seleccionadas</button></div>`,{onMount:(ov,close)=>{const b=ov.querySelector('#payCxp');if(b)b.addEventListener('click',()=>{const ids=[...ov.querySelectorAll('.cxpChk:checked')].map(c=>c.dataset.id);let n=0;let blocked=0;ids.forEach(id=>{const r=CX.finStore.cxp(p.id).find(x=>x.id===id);if(!r)return;if(currencyOf(r)===PENDING_CURRENCY){blocked++;return;}CX.finStore.abonarCxp(p.id,id,r.saldo||0);n++;});close();draw();ui.toast(n+' CxP de meses anteriores pagada(s) · egreso(s) en Movimientos'+(blocked?(' · '+blocked+' bloqueada(s) sin moneda'):''),blocked?'warn':'ok',4000);});}});});
    ...
    if(pay)pay.addEventListener('click',()=>{
      ...
      ui.modal('Confirmar pago de lote',`<p style="font-size:12.5px;color:var(--t2);margin-bottom:12px">Vas a registrar internamente <b>${draft.length}</b> liquidación(es) por <b>${Object.keys(porMon).map(m=>ui.money(m,porMon[m])).join(' + ')}</b>. El registro durable no equivale a confirmación bancaria; la conciliación externa seguirá pendiente.</p>${restantes.length?`<label class="flex" style="gap:8px;font-size:12px;color:var(--t1);background:var(--amber-bg);padding:9px 11px;border-radius:9px;cursor:pointer"><input type="checkbox" id="difCxp" checked> Diferir las <b>${restantes.length}</b> validada(s) no incluida(s) a Cuentas por Pagar (cierre de mes)</label>`:''}<div style="text-align:right;margin-top:14px"><button class="btn btn-green btn-sm" id="confPay">Registrar pago interno</button></div>`,{onMount:(ov,close)=>{const conf=ov.querySelector('#confPay');conf.addEventListener('click',async()=>{
        if(conf.disabled)return;const priorLabel=conf.textContent;conf.disabled=true;conf.textContent='Registrando…';
        try{
          const ids=[...draft];
          ...
          CX.finStore.clearDraft(p.id);reviewIds.forEach(id=>CX.finStore.toggleDraft(p.id,id));
          try{if(CX.backend?.refresh)await CX.backend.refresh();}catch(_){/* committed; refresh best-effort */}
          close();
...
CX.module('lotes', ({data,ui})=>{
  ...
  const groupKey=(l)=>{
    ...
    if(!l.pais || !l.moneda){const seed=l.visitaId || [l.shopper||'',l.sucursal||'',l.total||0,l.fechaEstimadaPago||l.fechaPago||''].join('|');let h=0;for(let i=0;i<String(seed).length;i++) h=(h*31+String(seed).charCodeAt(i))|0;return 'revision::'+Math.abs(h).toString(36);}
    const f=l.fechaEstimadaPago||l.fechaPago||'—';const periodo=(f&&f!=='—')?f.slice(0,7):'—';return ['legacy',tenantId,p.id,l.pais,l.moneda,periodo,f].join('::');
  };
  const porLote={}; liqsPagadas.forEach(l=>{const k=groupKey(l);(porLote[k]=porLote[k]||[]).push(l);});
  const lotesReales=Object.keys(porLote).map((k,i)=>{const ls=porLote[k];const monto=ls.reduce((a,l)=>a+(l.total||0),0);const confirmado=ls.every(l=>l.estado==='pagada');const f=ls[0].fechaEstimadaPago||ls[0].fechaPago||'—';let h=0;for(let c=0;c<k.length;c++)h=(h*31+k.charCodeAt(c))|0;const conflict = ls[0].loteId && loteIdCombos[ls[0].loteId] && loteIdCombos[ls[0].loteId].size>1;const baseId = ls[0].loteId?('#'+ls[0].loteId):('#LOTE-'+Math.abs(h).toString(36).toUpperCase().slice(0,6));return {id: conflict?(baseId+' · '+(ls[0].pais||'—')+'/'+(ls[0].moneda||'—')):baseId,n:ls.length,monto,cur:(ls[0].moneda||(ls[0].pais&&p.currency&&p.currency[ls[0].pais])||'pending_currency'),pais:ls[0].pais,estado: conflict?'Revisión requerida':(ls[0].moneda||(ls[0].pais&&p.currency&&p.currency[ls[0].pais]))?(confirmado?'Pagado':'Pagado (preview)'):'Revisión requerida', tone: conflict?'r':((ls[0].moneda||(ls[0].pais&&p.currency&&p.currency[ls[0].pais]))?(confirmado?'g':'a'):'r'), confirmado, conflict,fecha:f,visitas:ls.slice(0,10).map(l=>[l.shopper||'—',l.sucursal||'—',l.total||0])}; });
  const lotes = _showFixturesLotes ? lotesDemo : lotesReales;
  const html=`${ui.ph('Lotes de Pago', p.name+' · agrupa liquidaciones validadas y crea el egreso')}${lotes.length?`<div class="grid g3" style="margin-bottom:16px">${lotes.map(r=>`<div class="card hov card-p" data-lote="${r.id}" style="cursor:pointer"><div class="between" style="margin-bottom:8px"><b style="font-family:var(--disp);font-size:15px;color:var(--t1)">${r.id}</b>${ui.bdg(r.estado,r.tone)}</div><div style="font-size:12px;color:var(--t3)">${r.n} visitas · ${r.fecha}</div><div style="font-size:18px;font-weight:800;color:var(--green);font-family:var(--disp);margin-top:4px">${r.cur===PENDING_CURRENCY?'<span class="bdg bdg-r">Pendiente de moneda · Revisión requerida</span>':_m(r.cur,r.monto)}</div><div style="margin-top:10px"><button class="btn btn-ghost btn-sm" data-lote="${r.id}">Ver detalle →</button></div></div>`).join('')}</div>`:`<div class="card card-p" style="margin-bottom:16px">${ui.degraded('Todavía no hay liquidaciones pagadas en este proyecto — los lotes se listan aquí cuando en Liquidaciones se paga un lote real.',{title:'Lotes de pago · pendiente de fuente'})}</div>`}<div class="card card-p">${ui.aiBox('Agrupo y concilio pagos automáticamente, evitando duplicidad. Cada lote crea su egreso asociado en Finanzas.','Conciliación')}</div>`;
  setTimeout(()=>{document.querySelectorAll('[data-lote]').forEach(b=>b.addEventListener('click',()=>{ const r=lotes.find(x=>x.id===b.dataset.lote); if(!r)return;const _loteReview=(r.cur===PENDING_CURRENCY)||r.estado==='Revisión requerida';ui.modal('Lote '+r.id+' · '+r.estado,`<div class="between" style="margin-bottom:12px"><div style="font-size:12.5px;color:var(--t2)">${r.n} visitas · ${r.fecha}</div><div style="font-size:17px;font-weight:800;color:var(--green);font-family:var(--disp)">${r.cur===PENDING_CURRENCY?'<span class="bdg bdg-r">Pendiente de moneda</span>':_m(r.cur,r.monto)}</div></div><table class="tbl"><thead><tr><th>Shopper</th><th>Sucursal</th><th style="text-align:right">Honorario</th></tr></thead><tbody>${r.visitas.map(v=>`<tr><td><b>${v[0]}</b></td><td style="font-size:12px">${v[1]}</td><td style="text-align:right;font-weight:700">${r.cur===PENDING_CURRENCY?'Pendiente de moneda':_m(r.cur,v[2])}</td></tr>`).join('')}${r.visitas.length<r.n?`<tr><td colspan="3" style="font-size:11px;color:var(--t3);text-align:center">+ ${r.n-r.visitas.length} visita(s) más en el lote</td></tr>`:''}</tbody></table><div style="margin-top:14px;display:flex;justify-content:flex-end;gap:8px">${_loteReview?ui.bdg('🔒 Revisión requerida · sin moneda · pago y export bloqueados','r'):(r.estado!=='Pagado'?`<button class="btn btn-green btn-sm" id="loteMark">Marcar pagado (vista previa)</button><button class="btn btn-ghost btn-sm" id="loteExp">⤓ Exportar</button>`:ui.bdg('✓ Egreso preparado · cruce real pendiente backend','g')+`<button class="btn btn-ghost btn-sm" id="loteExp">⤓ Exportar</button>`)}</div>`,{onMount:(ov,close)=>{ const lm=ov.querySelector('#loteMark'); if(lm)lm.addEventListener('click',()=>{if(r.cur===PENDING_CURRENCY||r.estado==='Revisión requerida'){ui.toast('Lote en revisión de moneda: no se puede marcar pagado','err');return;}if(!CX.permissions.gate('finance.markPaid',CX.permissions.ctx({entityType:'lote_pago',entityId:r.id}),ui)) return;close();ui.toast('Lote '+r.id+' marcado pagado (preview) · egreso reflejado en Movimientos · pendiente cruce financiero real','ok',36000);}); const le=ov.querySelector('#loteExp'); if(le)le.addEventListener('click',()=>{if(r.cur===PENDING_CURRENCY||r.estado==='Revisión requerida'){ui.toast('Lote en revisión de moneda: export bloqueado','err');return;}ui.toast('Exportando lote '+r.id+'…','ok');}); }}); }));},0);
  return html;
});
