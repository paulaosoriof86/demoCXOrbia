...
CX.module('automatizaciones', ({data,ui})=>{
  ...
  const draw=()=>{
    ...
    <div class="grid g2" style="gap:14px;margin-bottom:16px">
      <div class="card card-p">
        <div class="card-t" style="font-size:13px;margin-bottom:10px">🔗 Conexión con Make (escenario)</div>
        <div style="font-size:11.5px;color:var(--t2);margin-bottom:8px">Estado: ${ui.statusBdg(hookStatus)}</div>
        <label class="flex" style="gap:8px;font-size:12.5px;color:var(--t1);margin-bottom:6px"><input type="checkbox" id="hookReady" ${A.hookRequested()?'checked':''}> Marcar como listo para conectar (el backend pedirá la URL real por su propio canal seguro)</label>
        <div style="font-size:11px;color:var(--t3)">🔒 Este prototipo no pide ni guarda la URL real del webhook, ni por tenant ni por automatización individual — no hay ningún campo para pegarla. No se fabrica ninguna referencia local que aparente una conexión: <code>connectionRef</code> solo existe si lo emite un backend real, y aquí siempre es <code>null</code>.</div>
      ...
      <div class="card card-p">
        ...
        ${[['Make','Orquestador de escenarios',hookStatus==='pending_backend'?'Pendiente backend':'Configurar'],['Outlook / M365','Correo y calendario',aic._outlook?'Preparado (demo)':'Vincular'],['Gmail / Workspace','Correo',''],['Google Sheets','HR viva + export','Preparado (demo)'],['WhatsApp Cloud','Mensajería','vía Make (gate pendiente)']].map(r=>`<div class="between" style="padding:7px 0;border-bottom:1px solid var(--border-2)"><div><b style="font-size:12.5px">${r[0]}</b><div style="font-size:10.5px;color:var(--t3)">${r[1]}</div></div><button class="btn btn-ghost btn-sm" data-int="${r[0]}">${r[2]||'Vincular'}</button></div>`).join('')}
      </div>
    </div>
    ...
    <div class="card card-p" style="margin-bottom:16px">
      ...
      <div style="font-size:11px;color:var(--t3);margin-top:8px">Variables: {shopper} {sucursal} {fecha} {estado} {score}. La <b>escritura de vuelta a HR</b> mantiene la doble vía sin duplicar. El webhook de Make se gestiona a nivel de tenant (arriba), no por automatización individual.</div>
    </div>
    ...
    <div class="grid g2" style="gap:14px;margin-bottom:16px">
      ...
      <div class="card card-p">
        ...
        <div class="grid g2" style="gap:8px 12px">
          <div><label class="lbl">Modelo (preferencia — el backend real decide con qué conectarse)</label><select class="sel" id="aiModel">${modelOpts}</select></div>
          <div><label class="lbl">Estado de conexión</label><div class="inp" style="display:flex;align-items:center;color:var(--t3);font-size:12px">🔒 Solo backend/adapter — sin API key en el navegador</div></div>
        </div>
        ...
        <div style="font-size:11px;color:var(--amber);margin-top:6px">🔒 El navegador nunca llama al proveedor de IA ni guarda su API key — eso lo resuelve el backend/adapter autorizado. Aquí solo se guarda tu preferencia de modelo para cuando ese backend exista.</div>
        <div style="font-size:11px;color:var(--t3);margin-top:6px">Mientras tanto, los importadores y asistentes usan <b>heurística local</b> (sin costo ni llamada externa). El modelo que elijas aplica a TODAS las funciones inteligentes cuando el backend esté conectado.</div>
    ...
    <div class="card card-p">
      <div class="card-h"><div class="card-t">📜 Outbox preparado (envío real vía Make pendiente backend)</div><span class="muted" style="font-size:11px">últimos eventos encolados · estado por entrega</span></div>
      ...
      <div style="font-size:10.5px;color:var(--t3);margin-top:8px">Ítem 13 (paquete genérico 20260711): cada fila del outbox ahora muestra su estado con el vocabulario único de ui.statusBdg — "Preparado (preview)" para plantillas de WhatsApp Web listas para abrir manualmente, "Pendiente de backend" para todo lo que espera el POST real a Make/correo/Sheets. Nunca dice "enviado" ni "entregado" porque este prototipo no ejecuta ningún envío real.</div>
  ...
  const bind=()=>{
    ...
    host.querySelector('#hookSave').addEventListener('click',()=>{if(!CX.permissions.gate('automation.configure',CX.permissions.ctx(),ui))return;const on=host.querySelector('#hookReady').checked;A.setHookRequested(on);ui.toast(on?'Marcado como listo para conectar (preview) · sin URL real guardada · pendiente backend':'Preferencia retirada','ok');draw();});
    host.querySelector('#hookTest').addEventListener('click',()=>{if(!CX.permissions.gate('integration.test',CX.permissions.ctx(),ui))return;const requested=A.hookRequested();A._pushLog({fecha:new Date().toISOString().slice(0,16).replace('T',' '),canal:'sheet',evento:'test',titulo:'Disparo de prueba (local)',txt:'Payload de prueba preparado para el escenario Make · envío real pendiente backend',estado:requested?'pending_backend':'not_requested'});draw();ui.toast(requested?'Disparo simulado (preview) · envío real pendiente backend/Make':'Marca la preferencia de conexión primero','ok');});
    host.querySelectorAll('[data-int]').forEach(b=>b.addEventListener('click',()=>{if(!CX.permissions.gate('integration.configure',CX.permissions.ctx(),ui))return;if(b.dataset.int==='Outlook / M365'){AI.save({_outlook:true});ui.toast('Outlook preparado (demo) · conexión real de correo y calendario pendiente backend','ok');draw();}else ui.toast(b.dataset.int+': preparado (demo) · conexión real pendiente backend','ok');}));
