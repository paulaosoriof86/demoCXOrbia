...
Responde SOLO con JSON válido con este formato exacto:
{
  ...
  const draw=()=>{
    ...
    <div style="font-size:10.5px;color:var(--t3);margin:-8px 0 10px">${(()=>{const c=data.ctx?data.ctx():null;const sc=(CX.dataSource&&CX.dataSource.sourceContract)?CX.dataSource.sourceContract():null;const parts=[];if(c)parts.push('destino: '+c.projectId+' · '+c.tenantId);if(sc)parts.push('fuente actual: '+sc.sourceReadMode);return parts.join(' · ');})()}</div>
