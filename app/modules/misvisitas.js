...
CX.module('misvisitas',({data,ui})=>{
  ...
  const pendingApps=identityOk?(data._posts||[]).filter(x=>String(x.shopperId||'')===String(sid)&&postPeriod(x)===String(data.currentPeriodId)&&postState(x)==='pendiente'):[];
  ...
  const geoBtn=v=>{if(!geoOn||!v)return'';const g=geoPreview.get(v.id);return g?`<button class="btn btn-soft btn-sm" data-geo="${v.id}" title="Check-in preparado solo en esta vista; guardado de evidencia pendiente">⏳ Check-in ${g.ts||''}</button>`:`<button class="btn btn-green btn-sm" data-geo="${v.id}">📍 Check-in geolocalizado</button>`;};
  ...
  const tabs=()=>`<div class="flex" style="margin-bottom:14px;gap:8px;flex-wrap:wrap"><button class="btn btn-sm ${view==='activas'?'btn-pr':'btn-ghost'}" data-view="activas">Activas ${activeCount}</button><button class="btn btn-sm ${view==='historial'?'btn-pr':'btn-ghost'}" data-view="historial">Historial ${history.length}</button><span class="bdg ${pendingApps.length?'bdg-a':'bdg-n'}">Postulaciones pendientes ${pendingApps.length}</span></div>`;
  const pendingAppsHTML=()=>pendingApps.length?`<div class="card card-p" style="margin-bottom:12px;border-left:3px solid var(--amber)"><div class="card-h"><div class="card-t">Postulaciones pendientes</div><span class="bdg bdg-a">${pendingApps.length}</span></div><div style="font-size:11.5px;color:var(--t2);margin-bottom:8px">Estas postulaciones están persistidas y pendientes de decisión. No cuentan como visitas activas hasta que exista asignación confirmada.</div>${pendingApps.map(a=>{const v=(data._visitas||[]).find(x=>String(x.id||x.visitId)===String(a.visitaId||a.visitId));return`<div class="between" style="padding:7px 0;border-top:1px solid var(--border-2)"><div><b style="font-size:12.5px">${v?.sucursal||a.sucursal||'Visita'}</b><div style="font-size:10.5px;color:var(--t3)">${a.fechaProp||a.proposedDate||'Fecha pendiente'} · ${v?.ciudad||a.ciudad||''}</div></div>${ui.bdg('Pendiente','a')}</div>`;}).join('')}</div>`:''; 
  const activeHTML=()=>`${ui.ph('Mis Visitas',(p?.name||'')+' · agenda, ejecuta y da seguimiento')}${tabs()}${pendingAppsHTML()}${geoOn?`<div class="card card-p" style="margin-bottom:12px;border-left:3px solid var(--green)"><div class="flex" style="gap:8px;align-items:center;margin-bottom:4px"><span style="font-size:18px">📍</span><b style="font-size:13px">Check-in con foto geolocalizada</b><span class="bdg bdg-a" style="font-size:10px">Evidencia pendiente</span></div><div style="font-size:11.5px;color:var(--t2)">Puedes preparar la foto y ubicación, pero la evidencia no quedará registrada hasta que esta función esté habilitada.</div></div>`:''}${assigned.map(visitCard).join('')}${scheduled.map(visitCard).join('')}${realized.map(visitCard).join('')}${activeCount?'':ui.empty('🗓️','No tienes visitas activas en este momento.')}<div class="card card-p">${ui.aiBox('Cada cambio se muestra como confirmado únicamente cuando queda guardado correctamente. Si no puede guardarse, tu visita permanece sin cambios.','Seguimiento seguro de la visita')}</div>`;
  const historyHTML=()=>`${ui.ph('Mis Visitas',(p?.name||'')+' · agenda, ejecuta y da seguimiento')}${tabs()}<div class="card card-p"><div class="card-h"><div class="card-t">Historial de visitas</div><span class="muted" style="font-size:11px">submitidas, liquidadas, pagadas o canceladas</span></div>${history.length?`<div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Sucursal</th><th>Escenario</th><th>Fecha</th><th>Honorario</th><th>Estado</th><th>Pago</th></tr></thead><tbody>${history.map(v=>{const vc=contract(v);const pay=!vc||vc.paymentState==='no_aplica'?'<span class="muted" style="font-size:11px">—</span>':vc.paymentState==='confirmado'?ui.bdg('Pagado (confirmado)','g'):ui.bdg('Pago pendiente de confirmación','a');return`<tr><td><b>${v.sucursal}</b><div style="font-size:10px;color:var(--t3)">${CX.paisFlag(v.pais)} ${v.ciudad||''}</div></td><td style="font-size:12px">${v.escenario||''}</td><td style="font-size:12px">${v.realizada||v.fechaPago||v.agendada||'—'}</td><td>${ui.money(v.currency,v.honorario)}</td><td>${ui.estadoBadge(v.estado)}</td><td>${pay}</td></tr>`;}).join('')}</tbody></table></div>`:ui.empty('🗒️','Aún no tienes visitas en tu historial.')}</div>`;
  ...
  const bindActive=()=>{
    ...
    host.querySelectorAll('[data-geo]').forEach(b=>b.addEventListener('click',()=>{
      ...
      ui.modal('📍 Check-in geolocalizado · '+v.sucursal,`<div style="background:var(--amber-bg,#fffbeb);border-radius:9px;padding:9px 12px;font-size:11.5px;color:#8a5b00;margin-bottom:12px">⏳ <b>Guardado de evidencia no disponible.</b> La preparación queda solo en esta vista y no altera la visita.</div><input type="file" accept="image/*" capture="environment" class="inp" id="geoPhoto" style="padding:6px;font-size:12px;margin-bottom:8px"><div id="geoPrev" style="margin-bottom:10px"></div><button class="btn btn-soft btn-sm" id="geoLoc" type="button" style="width:100%;justify-content:center;margin-bottom:6px">${prior?'⏳ '+prior.ts:'📍 Capturar ubicación GPS'}</button><div id="geoMsg" style="font-size:11px;color:var(--t3);min-height:16px"></div><div style="text-align:right;margin-top:12px"><button class="btn btn-pr btn-sm" id="geoSave" disabled>Preparar check-in</button></div>`,{onMount:(ov,close)=>{
        ...
        save.addEventListener('click',()=>{if(!(cap&&photoOk))return;geoPreview.set(v.id,{lat:cap.lat,lon:cap.lon,ts:cap.ts,acc:cap.acc,pending:true});if(previewUrl)URL.revokeObjectURL(previewUrl);close();draw();ui.toast('Check-in preparado temporalmente · la evidencia aún no ha sido guardada','warn',3600);});
      }});
    }));
