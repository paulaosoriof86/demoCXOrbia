/* CXOrbia — reusable Shopper admin command contract v1.
   Source-only. Defines durable create/update semantics without executing provider writes.
...
(function(root){
  ...
  const exactKeys=()=>Array.isArray(root.CX_EXACT_IDENTITY_CONTRACT?.technicalKeys)
    ?root.CX_EXACT_IDENTITY_CONTRACT.technicalKeys.slice()
  ...
  function safeProfile(input){
    input=input||{};
    const allowed=['firstName','lastName','nombre','email','whatsapp','pais','country','depto','ciudad','sexo','edad','estado','sourceRef','sourceType'];
    const out={};
  ...
  function exactAnchors(input){
    ...
    exactKeys().forEach(key=>{
      ...
      if(Array.isArray(value)){const values=uniq(value);if(values.length)out[key]=values;}
      else if(str(value))out[key]=str(value);
    });
    return out;
  }
  ...
  function create(input){
    ...
    const profile=safeProfile(input.profile||input);
    ...
    return {
      ...
      command:{
        ...
        projectId:str(input.projectId||uniq(input.projectIds)[0]),
        ...
        payload:{
          ...
          profile,
          exactIdentityAnchors:exactAnchors(input.identity||input),
          auth:{
            namespace:'shopper',
            principalRequired:true,
            claimsRequired:true,
            membershipRequired:true,
            credentialMode:'server_generated_or_protected_reset_flow',
            browserPasswordAllowed:false,
            browserTokenAllowed:false
          },
          persistence:{profileRequired:true,crosswalkRequired:true,providerAckRequired:true,localStorageAllowed:false},
          sourceType:str(input.sourceType||profile.sourceType||'platform'),
          sourceRef:str(input.sourceRef||profile.sourceRef||'')||null
        },
        source:'admin-shopper-flow'
      }
    };
  }
  ...
  function update(input){
    input=input||{};
    const errors=validateBase(input);
    const shopperId=str(input.shopperId||input.entityId);
    if(!shopperId)errors.push('missing-shopperId');
    return {
      ok:errors.length===0,
      errors,
      command:{
        commandType:'shopper.update',
        entityType:'shopper',
        entityId:shopperId||null,
        tenantId:str(input.tenantId),
        projectId:str(input.projectId||uniq(input.projectIds)[0]),
        actor:{actorId:str(input.actorId||''),role:str(input.actorRole),projectIds:uniq(input.projectIds)},
        expectedVersion:input.expectedVersion,
  ...
  function credentialReset(input){
    input=input||{};
    const shopperId=str(input.shopperId);
    return {
      ok:Boolean(str(input.tenantId)&&shopperId&&str(input.idempotencyKey)&&str(input.actorRole)),
      errors:[
        !str(input.tenantId)?'missing-tenantId':null,
        !shopperId?'missing-shopperId':null,
        !str(input.idempotencyKey)?'missing-idempotencyKey':null,
        !str(input.actorRole)?'missing-actorRole':null
      ].filter(Boolean),
      command:{
        commandType:'shopper.credential.reset',
        entityType:'shopper',
        entityId:shopperId||null,
        tenantId:str(input.tenantId),
        ...
        idempotencyKey:str(input.idempotencyKey),
        payload:{shopperId,serverOnly:true,browserPasswordAllowed:false,browserTokenAllowed:false},
        source:'admin-shopper-credential-reset'
      }
    };
  }

  CX.shopperAdminCommandContract=Object.freeze({
    version:VERSION,
    create,
    update,
    credentialReset,
    exactIdentityKeys:exactKeys(),
    browserCredentialStorageAllowed:false,
    localStoragePersistenceAllowed:false,
