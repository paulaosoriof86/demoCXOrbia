...
(function(root){
  ...
  function scope(input){
    ...
    return {
      tenantId:str(input?.tenantId||a.tenantId||CX.BACKEND?.tenantId),
      projectId:str(input?.projectId||period?.rootProjectId||d.currentProjectId),
      periodId:str(input?.periodId||period?.id||period?.periodId||d.currentPeriodId),
      actor:a
    };
  }
  function idem(prefix,parts){
    return [prefix,...arr(parts).map(str).filter(Boolean)].join(':');
  }
  ...
  async function createApplication(input){
    ...
    if(!s.tenantId||!s.projectId||!s.periodId)return {ok:false,status:'blocked',code:'OPERATIONAL_SCOPE_REQUIRED'};
    return execute({
      commandType:'application.create',entityType:'application',entityId:null,tenantId:s.tenantId,projectId:s.projectId,periodId:s.periodId,
      expectedVersion:'absent',idempotencyKey:str(input.idempotencyKey||idem('application.create',[s.tenantId,s.projectId,s.periodId,visitId,shopperId])),
      payload:{periodId:s.periodId,visitId,shopperId,proposedDate:input.proposedDate||input.fechaProp||null,note:input.note||null,hrRowId:visit?.hrRowId||null},
  ...
  async function updateApplicationStatus(input){
    ...
    if(!s.tenantId||!s.projectId||!s.periodId)return {ok:false,status:'blocked',code:'OPERATIONAL_SCOPE_REQUIRED'};
    return execute({
      commandType:'application.status.update',entityType:'application',entityId:applicationId,tenantId:s.tenantId,projectId:s.projectId,periodId:s.periodId,
      expectedVersion:input.expectedVersion??versionOf(application),idempotencyKey:str(input.idempotencyKey||idem('application.status.update',[s.tenantId,s.projectId,s.periodId,applicationId,input.status])),
      payload:{periodId:s.periodId,status:str(input.status).toLowerCase(),reason:input.reason||null,visitId:input.visitId||application?.visitId||application?.visitaId||null},
  ...
  async function deleteApplication(input){
    input=input||{};const s=scope(input),application=input.application||arr(CX.data?._posts).find(p=>str(p.id||p.applicationId||p.postulationId)===str(input.applicationId));
    const applicationId=str(input.applicationId||application?.id||application?.applicationId||application?.postulationId);
    if(!applicationId)return {ok:false,status:'blocked',code:'APPLICATION_ID_REQUIRED'};
    if(isSyntheticHrPost(application))return {ok:false,status:'blocked',code:'SYNTHETIC_HR_APPLICATION_DELETE_FORBIDDEN'};
    if(!s.tenantId||!s.projectId||!s.periodId)return {ok:false,status:'blocked',code:'OPERATIONAL_SCOPE_REQUIRED'};
    return execute({
      commandType:'application.delete',entityType:'application',entityId:applicationId,tenantId:s.tenantId,projectId:s.projectId,periodId:s.periodId,
      expectedVersion:input.expectedVersion??versionOf(application),idempotencyKey:str(input.idempotencyKey||idem('application.delete',[s.tenantId,s.projectId,s.periodId,applicationId])),
      payload:{periodId:s.periodId,applicationId,visitId:input.visitId||application?.visitId||application?.visitaId||null,reason:input.reason||null},
      source:'cx.data',authorization:{providerEnforcementRequired:true,permission:'application.delete'},
      reason:'application_delete_durable'
    });
  }
  ...
  async function assignVisit(input){
    ...
    if(!s.tenantId||!s.projectId||!s.periodId)return {ok:false,status:'blocked',code:'OPERATIONAL_SCOPE_REQUIRED'};
    return execute({
      commandType:'visit.assign',entityType:'visit',entityId:visitId,tenantId:s.tenantId,projectId:s.projectId,periodId:s.periodId,
      expectedVersion:input.expectedVersion??versionOf(visit),idempotencyKey:str(input.idempotencyKey||idem('visit.assign',[s.tenantId,s.projectId,s.periodId,visitId,shopperId])),
      payload:{periodId:s.periodId,visitId,shopperId,hrRowId:visit?.hrRowId||null,assignmentSource:str(input.assignmentSource||'platform'),assignmentSyncStatus:str(input.assignmentSyncStatus||'pending_hr')},
  ...
  function reconcileAssignment(platformVisit,hrVisit){
    ...
    const pk=[p.tenantId,p.rootProjectId||p.projectId,p.periodId,p.id||p.visitId,p.hrRowId].map(str);
    const hk=[h.tenantId,h.rootProjectId||h.projectId,h.periodId,h.id||h.visitId,h.hrRowId].map(str);
    if(pk.some(v=>!v)||hk.some(v=>!v))return {decision:'review_conflict',conflict:true,code:'stable_identity_missing',automaticOverwrite:false};
  ...
  function installCommandFacade(){
    ...
    d.deleteApplication=deleteApplication;
  ...
  function activate(reason){
    ...
    root.CX_TYA_OPERATIONAL_AUTHORITY_LOCK={
      ...
      assignmentKey:['tenantId','projectId','periodId','visitId/hrRowId','shopperId'],
      dedupeByName:false,
  ...
  root.CX_TYA_PHASE_A_OPERATIONAL_SYNC=Object.freeze({
    ...
    createApplication,updateApplicationStatus,deleteApplication,assignVisit,
