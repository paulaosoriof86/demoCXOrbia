...
(function(){
  ...
  const shopperCredentialRule=s=>credentialApi?.shopperCredentialRule?.(s)||{ok:false};
  const certStatus=s=>s&&s.certificationStatus||((s&&s.certified)?'certificada':(s&&s.certificationPresented)?'presentada':'sin_registro');
  const certEvidence=s=>arr(s&&s.certificationEvidenceRecords);
  ...
  function recalcProfile(s){
    ...
    if(credential.ok){
      ...
      s.credentialRuleVersion=credentialApi?.CREDENTIAL_RULE_VERSION||'';
    }
  ...
  function installCanonicalDataFacade(){
    ...
    d.shopperHistoryVisits=function(id,onlyCurrentProject){const vs=this.visitsForShopper(id,onlyCurrentProject),api=window.CX_TYA_CUMULATIVE_READ_MODEL;return api&&typeof api.confirmedHistoryVisits==='function'?api.confirmedHistoryVisits(vs):vs.filter(v=>v&&v.__pendingPlatformAssignmentOverlay!==true);};
    d.shopperStats=function(id){const vs=this.shopperHistoryVisits(id,false),fs=vs.map(facets);const s=this.getShopper?this.getShopper(str((this.__identityMap||{})[str(id)]||id)):null;return {total:vs.length,realizadas:fs.filter(f=>f.realized).length,liquidadas:fs.filter(f=>f.liquidationConfirmed).length,enCurso:fs.filter(f=>f.assigned&&!f.liquidationConfirmed&&!f.paymentConfirmed&&!f.cancelled).length,postulaciones:(this.postsForShopper?this.postsForShopper(id):[]).length||Number(s&&s.postulaciones||0),submitted:fs.filter(f=>f.submitted).length,paymentConfirmed:fs.filter(f=>f.paymentConfirmed).length};};
    ...
    d.certificationForShopper=function(id){const s=this.getShopper?this.getShopper(str((this.__identityMap||{})[str(id)]||id)):null;if(!s)return {status:'sin_registro',presented:false,certified:false,records:[],historicalEvidence:[],historicalEvidenceStatus:'none',eligibilityGranted:false};const evidence=certEvidence(s);return {status:certStatus(s),presented:!!s.certificationPresented,certified:!!s.certified,records:arr(s.certificationRecords),historicalEvidence:evidence,historicalEvidenceStatus:evidence.length?'historical_review_pending':'none',eligibilityGranted:!!s.certified};};
  }
  ...
  function patchShoppers(){
    ...
    for(const tr of document.querySelectorAll('#shBody [data-sid]')){const s=CX.data.getShopper?.(tr.dataset.sid);if(!s)continue;const cells=tr.querySelectorAll('td');if(cells[3])cells[3].innerHTML=s.perfilCompleto?'<span class="bdg bdg-g">Completo</span>':'<span class="bdg bdg-a">Incompleto</span>';if(cells[4]){const st=certStatus(s),ev=certEvidence(s);cells[4].innerHTML=st==='certificada'?'<span class="bdg bdg-g">Certificado</span>':st==='presentada'?'<span class="bdg bdg-b">Certificación presentada</span>':ev.length?'<span class="bdg bdg-a">Histórico en revisión</span>':'<span class="bdg bdg-n">Sin certificación</span>';}}
  }
  function watchProfileModals(){
    const observer=new MutationObserver(()=>{for(const modal of document.querySelectorAll('.cx-modal:not([data-c6-profile-checked])')){modal.dataset.c6ProfileChecked='1';const title=str(modal.querySelector('.cx-modal-h .card-t')?.textContent);const matches=arr(CX.data?.shoppers).filter(s=>str(s.nombre)===title);if(matches.length!==1)continue;const s=matches[0],body=modal.querySelector('.cx-modal-b');if(!body)continue;const cs=certStatus(s),ev=certEvidence(s),box=document.createElement('div');box.className='card card-p';box.style.margin='10px 0';box.style.background='var(--panel-2)';box.innerHTML=`<div class="between"><b style="font-size:12.5px">Certificación del proyecto</b><span class="bdg bdg-${cs==='certificada'?'g':cs==='presentada'?'b':'n'}">${cs==='certificada'?'Certificada':cs==='presentada'?'Presentada':'Sin certificación vigente'}</span></div>${ev.length?'<div style="font-size:11.5px;color:var(--t2);margin-top:7px"><b>Evidencia histórica encontrada:</b> '+ev.length+' registro(s) con identidad técnica exacta · pendiente de validación; no habilita elegibilidad vigente.</div>':'<div style="font-size:11px;color:var(--t3);margin-top:5px">Sin evidencia histórica exacta asociada.</div>'}`;body.insertBefore(box,body.children[2]||null);}}
