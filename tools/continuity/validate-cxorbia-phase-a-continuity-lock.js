...
// Terminal receipts retain the epoch in which they were closed. They must share the same plan,
// but a later bounded subgate must not rewrite or invalidate already-frozen terminal evidence.
...
if(lock.currentIteration==='I5-G2'){
  ...
  if(a?.status==='PASS'){
    ...
    if(g2a.safety?.businessDataWritesAuthorized!==false||g2a.safety?.rebuild!==false||g2a.safety?.merge!==false)fail('G2-A safety contract invalid');
    if(lock.hardStops?.rerunG2AWithoutNewP0!==false)fail('G2-A anti-rerun stop missing');
  }
  const b=lock.g2Acceptance?.liveInPlatformSyntheticAcceptance;
  if(b?.status==='PENDING_NARROW_WRITE_AUTHORIZATION'){
    if(b.sameProductionUrlRequired!==true||b.alternatePlatformAllowed!==false||b.localCloneOrEmulatorAllowed!==false||b.syntheticDataOnly!==true||b.cleanupAndPostCleanReadbackRequired!==true||b.businessDataWritesAuthorizedNow!==false||b.externalHrWritesAuthorizedNow!==false||b.paymentExecutionAuthorizedNow!==false||b.makeGeminiAuthorizedNow!==false||b.authUserCreateOrResetAuthorizedNow!==false)fail('G2-B pending authorization boundary invalid');
  }
}
