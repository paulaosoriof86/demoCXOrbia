...
if(lock.m3ExecutionControl?.mechanismEpoch!==E||Number(lock.m3ExecutionControl?.currentResidualHolds)!==current||lock.m3ExecutionControl?.next!==next||lock.masterPlan?.next!==next)fail('lock_m3_control');
...
if(m3.m3MechanismEpoch!==E||!String(m3.status||'').startsWith('ACTIVE_MECHANISM_')||Number(m3.f1?.currentResidualHolds)!==current||m3.next!==next)fail('m3_evidence');
