...
if(lock.m3MechanismEpoch!==E||lock.m3ExecutionControl?.mechanismEpoch!==E||Number(lock.m3ExecutionControl?.currentResidualHolds)!==28)fail('m3_lock_state');
...
if(ledger.m3MechanismEpoch!==E||ledger.policy?.consumedRequestsAreImmutable!==true||ledger.policy?.rerunSameRequestIdAllowed!==false||ledger.policy?.neverExecutedHistoricalAuthorityMustNotBeMarkedConsumed!==true)fail('ledger_policy');
...
if(m3.m3MechanismEpoch!==E||cert.m3MechanismEpoch!==E||tomb.m3MechanismEpoch!==E||authority.m3MechanismEpoch!==E||Number(m3.f1?.currentResidualHolds)!==28||Number(tomb.progress?.currentResidualHolds)!==28)fail('m3_mechanism_state');
...
console.log('M1=CLOSED_PASS');console.log('M2=CLOSED_PASS');console.log('M3=MECHANISM_REPAIR_APPLIED_CERTIFICATION_PENDING_READBACK');
