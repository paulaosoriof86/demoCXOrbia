...
const compatible=(a,b)=>!a||!b||fold(a)===fold(b);
const intersect=(a,b)=>[...a].some(v=>b.has(v));
...
async function readHrPrivateIdentity(){const token=await oauthToken();const meta=await sheetsGet('?fields=sheets(properties(title,index))',token);const tabs=(meta.sheets||[]).map(s=>parseTab(s.properties?.title)).filter(Boolean).filter(t=>t.key>='2025-06'&&t.key<='2026-07');const out=new Map();for(const tab of tabs){const range=encodeURIComponent(`'${tab.title.replace(/'/g,"''")}'!A1:AI140`);const data=await sheetsGet(`/values/${range}?majorDimension=ROWS&valueRenderOption=FORMATTED_VALUE`,token);const values=data.values||[],hi=headerIndex(values);if(hi<0)continue;const header=values[hi]||[],shopperIdx=indexOfHeader(header,['Shopper Asignado']),cityIdx=indexOfHeader(header,['CIUDAD']),shoppingIdx=indexOfHeader(header,['Shopping']),countryIdx=indexOfHeader(header,['País']);for(let ri=hi+1;ri<values.length;ri++){const row=values[ri]||[],rawShopper=cell(row,shopperIdx),shopping=cell(row,shoppingIdx),city=cell(row,cityIdx);if(!shopping||!city||!isAssigned(rawShopper))continue;const country=countryIdx>=0?cell(row,countryIdx)||tab.country:tab.country;const plannedId=`shopper_${String(country||tab.country).toLowerCase()}_${sha(lower(rawShopper)).slice(0,10)}`;if(!unresolvedIds.has(plannedId))continue;const rec=out.get(plannedId)||{plannedShopperId:plannedId,name:rawShopper,nameKey:fold(rawShopper),country:country||tab.country,cities:new Set(),rows:0,periods:new Set()};if(fold(rec.name)!==fold(rawShopper))throw new Error(`hr_identity_hash_collision_${plannedId}`);if(city)rec.cities.add(fold(city));rec.rows++;rec.periods.add(tab.key);out.set(plannedId,rec);}}
return out;}
...
for(const source of [...hr.values()].sort((a,b)=>a.plannedShopperId.localeCompare(b.plannedShopperId))){
  const legacyMatches=(legacyByName.get(source.nameKey)||[]).filter(l=>compatible(source.country,l.country));
  const canonicalProfileMatches=(canonicalByName.get(source.nameKey)||[]).filter(c=>compatible(source.country,c.country));
  const canonicalVisitMatches=(visitsByName.get(source.nameKey)||[]).filter(v=>compatible(source.country,v.country));
