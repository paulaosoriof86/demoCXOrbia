...
function verifyInventoryAndCounts(inventory,tabs,fetches,periods,visits,diagnostics,counts){
  ...
  const unresolvedHeaders=diagnostics.filter(item=>!item.headerVariant);
  ...
  if(julyGt?.headerVariant!=='tab_scoped_compact')issues.push({code:'july_26_variant_mismatch',observed:julyGt?.headerVariant||null});
  if(julyHn?.headerVariant!=='full_identity')issues.push({code:'july_26_hn_variant_mismatch',observed:julyHn?.headerVariant||null});
...
async function main(){
  ...
  const inventoryVerification=verifyInventoryAndCounts(inventory,tabs,fetches,periods,visits,diagnostics,counts);
