...
/* CXOrbia · Corte 4 · Hosting DEV visual preflight/build.
   ...
   Provider reads + local build only. No provider write is executed here. */
...
const report={schemaVersion:'1.1.0',gate:'cxorbia-corte4-hosting-dev-visual-preflight',generatedAt:new Date().toISOString(),decision:'HOLD_NOT_EXECUTED',authorization:{id:authorizationId,exactTextMatched:false,hostingDeployAllowed:false,maxDeployExecutions:0},target:{projectId,displayNameVerified:false,firebaseVerified:false},source:{branch:process.env.GITHUB_REF_NAME||'',commit:process.env.GITHUB_SHA||'',protectedSmokeCommit:request.protectedSmokeCommit||'',runtimeUnchangedSinceProtectedSmoke:false},provider:{webAppVerified:false,firestoreEmpty:null,authUsers:null,emailPasswordEnabled:null,rulesVerified:false,defaultHostingSite:null},build:{prepared:false,publicDir:null,configPath:null,configInjected:false,proofCreated:false,credentialsEmbedded:false},safety:{hostingOnly:true,firestoreDocumentWrites:0,authUserWrites:0,authConfigWrites:0,storageWrites:0,rulesDeploys:0,functionsDeploys:0,imports:0,hrWrites:0,make:0,gemini:0,payments:0,merge:false,production:false,secretsOutput:false}};
...
function runtimeUnchanged(){const base=String(request.protectedSmokeCommit||'');if(!/^[0-9a-f]{40}$/i.test(base))stop('protected_smoke_commit_invalid');if(spawnSync('git',['merge-base','--is-ancestor',base,'HEAD'],{stdio:'ignore'}).status!==0)stop('protected_smoke_not_ancestor');const diff=spawnSync('git',['diff','--quiet',base,'HEAD','--','app',':!app/docs/**'],{stdio:'ignore'});report.source.runtimeUnchangedSinceProtectedSmoke=diff.status===0;if(!report.source.runtimeUnchangedSinceProtectedSmoke)stop('runtime_changed_since_protected_smoke');}
...
function prepareSite(cfg){
  ...
  const proof={schemaVersion:'1.1.0',authorizationId,environment:'DEV',cut:4,mode:'read-only-visual',firebaseProjectId:projectId,sourceCommit:process.env.GITHUB_SHA||'',protectedSmokeCommit:request.protectedSmokeCommit,hostingOnly:true,readOnly:true,writeMode:'disabled',firestoreDocumentWrites:false,authUsersExpected:0,emailPasswordExpected:false,storageWrites:false,rulesDeploy:false,functionsDeploy:false,imports:false,hrWrites:false,make:false,gemini:false,payments:false,merge:false,production:false};fs.writeFileSync(path.join(siteDir,'corte4-hosting-proof.json'),JSON.stringify(proof,null,2)+'\n','utf8');
...
try{assertRequest();runtimeUnchanged();const cfg=await verifyProvider();prepareSite(cfg);report.decision='PASS_C4_HOSTING_DEV_PREFLIGHT_READY_FOR_ONE_DEPLOY';writeReport();console.log(JSON.stringify(report,null,2));}catch(e){if(!fs.existsSync(reportPath))writeReport();console.error(String(e?.message||e));process.exitCode=1;}
