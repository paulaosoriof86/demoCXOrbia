...
function saveReport() {
  ...
  const lines = [
    ...
    '## Gates','',
    ...(report.gates.length ? report.gates.map(gate => `- ${gate.status}: \`${gate.id}\` (${gate.exitCode})`) : ['- none']),'',
    '## Blockers','',
...
function readJson(rel){const abs=path.join(root,rel);if(!fs.existsSync(abs))throw new Error(`required_file_missing:${rel}`);return JSON.parse(fs.readFileSync(abs,'utf8'));}
function git(...args){const result=spawnSync('git',args,{cwd:root,encoding:'utf8',maxBuffer:10*1024*1024});if(result.status!==0)throw new Error(`git_failed:${args.join(' ')}:${(result.stderr||'').slice(0,500)}`);return String(result.stdout||'').trim();}
...
function executeGate(id,command,args,env={}){
  ...
  const result=spawnSync(command,args,{cwd:root,encoding:'utf8',env:{...process.env,...env},maxBuffer:30*1024*1024});
  ...
  const gate={id,command:[command,...args].join(' '),startedAt,finishedAt:new Date().toISOString(),exitCode:result.status??1,status:result.status===0?'PASS':'HOLD',logPath:path.relative(root,logPath).replaceAll('\\','/'),stdoutTail:stdout.slice(-1200),stderrTail:stderr.slice(-1200)};
  report.gates.push(gate);return gate;
}
async function waitForServer(url,timeoutMs=30000){const deadline=Date.now()+timeoutMs;let last='';while(Date.now()<deadline){try{const response=await fetch(url,{redirect:'manual'});if(response.ok)return;last=`HTTP_${response.status}`;}catch(error){last=String(error?.message||error);}await new Promise(resolve=>setTimeout(resolve,500));}throw new Error(`static_server_not_ready:${last}`);}

function runtimeInventoryStableGates(prefix){return [
  [`${prefix}-node-check-runtime-inventory-filter`,'node',['--check','tools/hr-source/tya-filter-source-safe-to-inventory-r20.mjs']],
  [`${prefix}-runtime-inventory-filter`,'node',['tools/hr-source/tya-filter-source-safe-to-inventory-r20.mjs','--input','app/data/tya-hr-source-safe-periods.js','--out','app/data/tya-hr-source-safe-periods.js','--inventory','backend/contracts/tya-hr-tab-inventory-r20-v1.json','--report-dir',`.tmp/${prefix}-runtime-inventory-filter-r20`]],
  [`${prefix}-stable-payload-gate`,'node',['tools/qa/tya-source-safe-stable-visit-payload-r20-gate.mjs','--input','app/data/tya-hr-source-safe-periods.js']]
];}
function stableVisitIdentityGates(prefix){return [
  ...
  [`${prefix}-stable-visit-apply-idempotent`,'node',['tools/hr-source/tya-stabilize-source-safe-visit-ids-r20.mjs','--input','app/data/tya-hr-source-safe-periods.js','--out','app/data/tya-hr-source-safe-periods.js','--report-dir',`.tmp/${prefix}-stable-visit-id-r20`]]
];}
...
async function runV174Profile(){
  ...
  const server=spawn('python3',['-m','http.server','4173','--bind','127.0.0.1','--directory','app'],{cwd:root,stdio:['ignore',serverLog,serverLog],detached:false});
  try{
    await waitForServer('http://127.0.0.1:4173/index.html');
    executeGate('tya-project-period-kpi-history-gate-r20','node',['tools/qa/tya-project-period-kpi-history-gate-r20.mjs','--base-url','http://127.0.0.1:4173/index.html?cxTyaPhaseA=1&r18d=visible','--out','.tmp/tya-project-period-kpi-history-r20']);
    ...
    executeGate('tya-v174-r20-source-lock-proposal','node',['tools/release/tya-v174-r20-source-lock-proposal.mjs']);
    executeGate('tya-v174-proposed-source-lock-verify','node',['tools/release/tya-v174-proposed-source-lock-verify.mjs','.tmp/cxorbia-readonly-post-gates/source-lock-proposal']);
  }finally{server.kill('SIGTERM');fs.closeSync(serverLog);}
}
...
function runCorte3FinancialProfile(){
  ...
  executeGate('tya-financial-workbook-live-hr-reconcile-r14c-current-r20','node',['tools/reconciliation/tya-financial-workbook-live-hr-reconcile-r14c.mjs','--hr','app/data/tya-hr-source-safe-periods.js','--financial','backend/config/phase-a-financial-workbook-source-safe-r14.json','--out','.tmp/phase-a-financial-r14c-live-hr']);
  executeGate('tya-corte3-financial-reconciliation-r20-gate','node',['tools/qa/tya-corte3-financial-reconciliation-r20-gate.mjs','--current','.tmp/phase-a-financial-r14c-live-hr/financial-live-hr-reconciliation-r14c.source-safe.json','--baseline','backend/config/phase-a-financial-live-hr-reconciliation-r14c.source-safe.json','--review','backend/contracts/tya-corte3-financial-r20-delta-review-v1.json','--out','.tmp/tya-corte3-financial-reconciliation-r20']);
}

async function main(){
  fs.mkdirSync(outDir,{recursive:true});
  const contract=readJson(contractPath),request=readJson(requestPath);
  report.enabled=request.enabled===true;report.requestId=request.requestId||null;report.profile=request.profile||null;report.expectedSourceHead=request.expectedSourceHead||null;report.requestCommitSha=git('rev-parse','HEAD');report.sourceHeadSha=git('rev-parse','HEAD^');
  if(!report.enabled){report.status='SKIPPED_DISABLED_REQUEST';return;}
  ...
  const allowedProfiles=Array.isArray(readonlyContract.profiles)?readonlyContract.profiles:[readonlyContract.profile].filter(Boolean);
...
main().catch(error=>{report.status='HOLD_READONLY_POST_GATES';report.blockers.push(String(error?.message||error));process.exitCode=1;}).finally(saveReport);
