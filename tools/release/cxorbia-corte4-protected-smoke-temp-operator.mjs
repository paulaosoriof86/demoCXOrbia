...
const report={
  schemaVersion:'1.1.0',gate:'cxorbia-corte4-protected-cxdata-smoke-temp-operator',generatedAt:new Date().toISOString(),decision:'HOLD_NOT_EXECUTED',
...
async function createTempUser(apiKey,password){const url=`https://identitytoolkit.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/accounts?key=${encodeURIComponent(apiKey)}`;const r=await adminRequest('POST',url,{email:tempEmail,password,emailVerified:true,disabled:false,displayName:'CXOrbia Corte 4 Temporary Smoke Operator'});report.writes.authUserCreateTransient+=1;if(!r.ok||!r.payload?.localId)throw new Error(`auth_user_create_${r.status}_${category(r.payload?.error?.message)}`);report.temporaryAuth.userCreated=true;writeJson(statePath,{schemaVersion:'1.0.0',projectId,tempEmail,localId:String(r.payload.localId),emailBefore:report.preflight.emailEnabledBefore,passwordRequiredBefore:report.preflight.passwordRequiredBefore});return String(r.payload.localId);}
async function setClaims(apiKey,localId){const r=await adminRequest('POST',`https://identitytoolkit.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/accounts:update?key=${encodeURIComponent(apiKey)}`,{localId,customAttributes:JSON.stringify({role:tempRole,tenantId}),returnSecureToken:false});report.writes.authClaimsTransient+=1;if(!r.ok)throw new Error(`auth_claims_${r.status}_${category(r.payload?.error?.message)}`);report.temporaryAuth.claimsSet=true;}
