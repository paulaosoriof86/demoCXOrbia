...
if(!fs.existsSync(path.join(appDir,fastTriggerSrc)))fail(`Fast trigger missing: ${fastTriggerSrc}`);
...
const fastTriggerCount=html.split(fastTriggerTag).length-1;
...
if(fastTriggerCount!==1)fail(`fast trigger tag count ${fastTriggerCount}/1`);
