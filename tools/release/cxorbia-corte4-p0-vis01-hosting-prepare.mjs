#!/usr/bin/env node
/* CXOrbia · Corte 4 · P0-C4-VIS-01 · Hosting DEV revalidation preflight/build.
...
const requestPath=path.resolve(process.env.CXORBIA_REQUEST_PATH||'.github/cxorbia-firebase-requests/corte4-p0-vis01-revalidate.json');
const credentialPath=process.env.GOOGLE_APPLICATION_CREDENTIALS;
const outDir=path.resolve(process.env.CXORBIA_OUT_DIR||'.tmp/corte4-p0-vis01');
const siteDir=path.join(outDir,'site');
const configPath=path.join(outDir,'firebase.corte4-p0-vis01.json');
...
const report={schemaVersion:'1.0.0',gate:'cxorbia-corte4-p0-vis01-preflight',generatedAt:new Date().toISOString(),decision:'HOLD_NOT_EXECUTED',authorization:{id:authorizationId,exactTextMatched:false,hostingDeployAllowed:false,maxDeployExecutions:0},target:{projectId,displayNameVerified:false,firebaseVerified:false},source:{branch:process.env.GITHUB_REF_NAME||'',commit:process.env.GITHUB_SHA||'',deployedBase:request.deployedBaseCommit||'',runtimeDelta:[]},provider:{webAppVerified:false,firestoreEmpty:null,authUsers:null,emailPasswordEnabled:null,rulesVerified:false,previousHostingProofVerified:false,defaultHostingSite:null},build:{prepared:false,publicDir:null,configPath:null,configInjected:false,proofCreated:false,credentialsEmbedded:false},safety:{hostingOnly:true,firestoreDocumentWrites:0,authUserWrites:0,authConfigWrites:0,storageWrites:0,rulesDeploys:0,functionsDeploys:0,imports:0,hrWrites:0,make:0,gemini:0,payments:0,merge:false,production:false,secretsOutput:false}};
...
function assertFocusedRuntimeDelta(){
  ...
  if(!/^[0-9a-f]{40}$/i.test(base)) stop('deployed_base_invalid');
  ...
  const allowed=['app/core/backend-config-preview-dev.js','app/core/backend-cxdata-readonly-corte4.js','app/core/backend-preview-status.js'].sort();
  report.source.runtimeDelta=files;
  if(JSON.stringify(files)!==JSON.stringify(allowed)) stop('runtime_delta_not_focused');
  for(const f of allowed){ if(spawnSync(process.execPath,['--check',f],{stdio:'ignore'}).status!==0) stop(`syntax_${path.basename(f)}`); }
  const cfg=fs.readFileSync('app/core/backend-config-preview-dev.js','utf8');
  const guard=fs.readFileSync('app/core/backend-cxdata-readonly-corte4.js','utf8');
  const status=fs.readFileSync('app/core/backend-preview-status.js','utf8');
  if(!cfg.includes("CX.dataSource.mode = 'connected'")||!cfg.includes('CX.BRAND.demoMode = false')) stop('preview_identity_fix_missing');
  if(!guard.includes("clearToBackendEmpty('awaiting-protected-read')")||!guard.includes("fallbackUsed:false")) stop('pre_render_fail_closed_missing');
  if(!status.includes("return 'firestore-read-error'")||!status.includes('Firestore protegido · sin fallback')) stop('status_fail_closed_fix_missing');
}
...
async function verifyPreviousHostingProof(){
  const url=`https://${projectId}.web.app/corte4-hosting-proof.json?baseline=${encodeURIComponent(String(request.deployedBaseCommit||''))}`;
...
function prepareSite(cfg){
  ...
  const p=path.join(siteDir,'core','backend-config.js');let text=fs.readFileSync(p,'utf8');text=text.replace('apiKey: null',`apiKey: ${JSON.stringify(String(cfg.apiKey))}`).replace('messagingSenderId: null',`messagingSenderId: ${JSON.stringify(String(cfg.messagingSenderId||''))}`).replace('appId: null',`appId: ${JSON.stringify(String(cfg.appId))}`).replace("configSource: 'repo-placeholder-new-empty-candidate'","configSource: 'runtime-provider-hosting-dev-corte4-p0-vis01'").replace('projectIdentityVerified: false','projectIdentityVerified: true').replace('emptyProjectVerified: false','emptyProjectVerified: true');if(text.includes('apiKey: null')||text.includes('appId: null'))stop('runtime_config_injection_failed');fs.writeFileSync(p,text,'utf8');
  fs.writeFileSync(path.join(siteDir,'core','backend-dev-auth.local.js'),"/* Corte 4 P0 VIS-01 revalidation: no credential embedded; Auth remains disabled and runtime must fail closed without demo fallback. */\n",'utf8');
  const proof={schemaVersion:'1.0.0',authorizationId,environment:'DEV',cut:4,p0:'P0-C4-VIS-01',mode:'read-only-visual-revalidation',firebaseProjectId:projectId,sourceCommit:process.env.GITHUB_SHA||'',deployedBaseCommit:request.deployedBaseCommit,hostingOnly:true,readOnly:true,writeMode:'disabled',expectedFallbackUsed:false,firestoreDocumentWrites:false,authUsersExpected:0,emailPasswordExpected:false,storageWrites:false,rulesDeploy:false,functionsDeploy:false,imports:false,hrWrites:false,make:false,gemini:false,payments:false,merge:false,production:false};fs.writeFileSync(path.join(siteDir,'corte4-p0-vis01-proof.json'),JSON.stringify(proof,null,2)+'\n','utf8');
...
try{assertRequest();assertFocusedRuntimeDelta();await verifyPreviousHostingProof();const cfg=await verifyProvider();prepareSite(cfg);report.decision='PASS_C4_P0_VIS01_PREFLIGHT_READY_FOR_ONE_DEPLOY';writeReport();console.log(JSON.stringify(report,null,2));}catch(e){if(!fs.existsSync(reportPath))writeReport();console.error(String(e?.message||e));process.exitCode=1;}
