...
for(const marker of ['Tipo de acceso','Usuario','Administración / Coordinación','Shopper / Evaluador','authNamespace','tenant + \'\\0\' + ns + \'\\0\' + normalized'])if(!authSource.includes(marker))stop(`credential_continuity_source_marker_missing:${marker}`);
...
const siteList=Array.isArray(sites.payload?.sites)?sites.payload.sites:[];if(!siteList.some(s=>String(s?.name||'').endsWith(`/sites/${expectedSite}`)))stop('existing_hosting_site_missing');
