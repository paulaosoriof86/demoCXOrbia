#!/usr/bin/env node
/* CXOrbia · Corte 4 · P0-C4-VIS-02 · Hosting DEV revalidation preflight/build.
...
const requestPath=path.resolve(process.env.CXORBIA_REQUEST_PATH||'.github/cxorbia-firebase-requests/corte4-p0-vis02-revalidate.json');
const credentialPath=process.env.GOOGLE_APPLICATION_CREDENTIALS;
const outDir=path.resolve(process.env.CXORBIA_OUT_DIR||'.tmp/corte4-p0-vis02');
const siteDir=path.join(outDir,'site');
const configPath=path.join(outDir,'firebase.corte4-p0-vis02.json');
...
function stop(message){report.decision='HOLD_C4_P0_VIS02_PREFLIGHT';report.error=String(message||'unknown').replace(/[^A-Za-z0-9_.:-]/g,'_').slice(0,180);writeReport();throw new Error(report.error);}
function assertRequest(){
  const exact={schemaVersion:'cxorbia.corte4-p0-vis02-revalidate.v1',enabled:true,authorizationSource:'current_conversation',authorizationText:'Autorizo corrección focalizada de P0-C4-VIS-02 y un único Hosting DEV de revalidación de Corte 4, sin data writes ni producción',authorizationId:'c4-p0-vis02-revalidate-20260729-01',repository:'paulaosoriof86/demoCXOrbia',branch:'docs-tya-v6-v71-audit',pullRequest:7,targetProjectId:'cxorbia-tya-dev-260729-c4',targetDisplayName:'CXOrbia TyA DEV Clean Corte 4',webAppDisplayName:'CXOrbia TyA DEV Corte 4',mode:'P0_C4_VIS02_EMPTY_BACKEND_SHELL_AND_ROLE_SWITCH_FIX_ONE_HOSTING_REVALIDATION',hostingOnly:true,production:false,merge:false};
...
function assertFocusedRuntimeDelta(){
  ...
  const allowed=['app/core/backend-corte4-empty-shell-guard.js','app/index-backend-dev.html'].sort();
  report.source.runtimeDelta=files;
  if(JSON.stringify(files)!==JSON.stringify(allowed)) stop('runtime_delta_not_focused');
  if(spawnSync(process.execPath,['--check','app/core/backend-corte4-empty-shell-guard.js'],{stdio:'ignore'}).status!==0) stop('syntax_empty_shell_guard');
  const guard=fs.readFileSync('app/core/backend-corte4-empty-shell-guard.js','utf8');
  const html=fs.readFileSync('app/index-backend-dev.html','utf8');
  for(const marker of ['isProtectedEmpty()','renderEmptyShell(role)','clearShellDom()','CX_C4_EMPTY_SHELL_STATE','programBase=function(p)','programKey=function(p)']) if(!guard.includes(marker)) stop(`guard_marker_missing_${marker.replace(/[^A-Za-z0-9]/g,'_')}`);
  if(!html.includes('<script src="core/backend-corte4-empty-shell-guard.js"></script>')) stop('guard_not_loaded');
  if(html.indexOf('backend-corte4-empty-shell-guard.js')>html.indexOf('<script src="app.js"></script>')) stop('guard_must_load_before_app_boot');
}
...
async function verifyPreviousHostingProof(){
  const url=`https://${projectId}.web.app/corte4-p0-vis01-proof.json?baseline=${encodeURIComponent(String(request.deployedBaseCommit||''))}`;
...
function prepareSite(cfg){
  ...
  const p=path.join(siteDir,'core','backend-config.js');let text=fs.readFileSync(p,'utf8');text=text.replace('apiKey: null',`apiKey: ${JSON.stringify(String(cfg.apiKey))}`).replace('messagingSenderId: null',`messagingSenderId: ${JSON.stringify(String(cfg.messagingSenderId||''))}`).replace('appId: null',`appId: ${JSON.stringify(String(cfg.appId))}`).replace("configSource: 'repo-placeholder-new-empty-candidate'","configSource: 'runtime-provider-hosting-dev-corte4-p0-vis02'").replace('projectIdentityVerified: false','projectIdentityVerified: true').replace('emptyProjectVerified: false','emptyProjectVerified: true');if(text.includes('apiKey: null')||text.includes('appId: null'))stop('runtime_config_injection_failed');fs.writeFileSync(p,text,'utf8');
  fs.writeFileSync(path.join(siteDir,'core','backend-dev-auth.local.js'),"/* Corte 4 P0 VIS-02 revalidation: no credential embedded; Auth remains disabled; empty backend shell must remain stable across role switches. */\n",'utf8');
  const proof={schemaVersion:'1.0.0',authorizationId,environment:'DEV',cut:4,p0:'P0-C4-VIS-02',mode:'empty-backend-shell-role-switch-revalidation',firebaseProjectId:projectId,sourceCommit:process.env.GITHUB_SHA||'',deployedBaseCommit:request.deployedBaseCommit,hostingOnly:true,readOnly:true,writeMode:'disabled',expectedProjects:0,expectedFallbackUsed:false,firestoreDocumentWrites:false,authUsersExpected:0,emailPasswordExpected:false,storageWrites:false,rulesDeploy:false,functionsDeploy:false,imports:false,hrWrites:false,make:false,gemini:false,payments:false,merge:false,production:false};fs.writeFileSync(path.join(siteDir,'corte4-p0-vis02-proof.json'),JSON.stringify(proof,null,2)+'\n','utf8');
