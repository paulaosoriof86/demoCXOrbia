...
/*
  ...
  adapter after core/shoppers-store.js so the visible UI uses TyA/Cinepolis data
  instead of generic demo seeds.

  The adapter must run after shoppers-store.js. That core normalizes seed shoppers
  and would otherwise turn protected references into apparently complete profiles
  by adding firstName/lastName/whatsapp/perfilCompleto. Running the adapter after
  that normalization preserves the protected source declaration without changing
  frontend/core source files.
...
(function(){
  ...
  const periods = snapshot.periods.map(p => ({
    ...
    sourceSafe:true, importStatus:'hr_snapshot_source_safe_not_imported', runtimeSyncActive:false,
  ...
  const visits = (snapshot.visits || []).map((v, idx) => ({
    ...
    escenario:v.escenario || v.tipoCompra || '', franja:v.franja || '', franjaCode:v.franjaCode || 'WK', canal:'Visita presencial',
    ...
    cuestFecha:v.cuestFecha || null, submit:!!v.submit, submittedAt:v.submittedAt || null,
    assignmentSource:v.hasShopper ? 'hr' : null, assignmentSyncStatus:'hr_live_source_safe_preview', reviewRequired:false,
    sourceSafe:true, piiProtected:true
  }));

  const shoppers = (snapshot.shoppers || []).map((s, idx) => ({
    id:s.id || ('shopper-protegido-' + (idx + 1)),
    shopperId:s.shopperId || s.id || ('shopper-protegido-' + (idx + 1)),
    ...
    sourceHistoricalVisitCount:Number(s.visitas || 0),
    sourceHistoricalRealizedCount:Number(s.realizadas || 0),
    sourceHistoricalLiquidatedCount:Number(s.liquidadas || 0),
    visitas:undefined,
    realizadas:undefined,
  ...
  CX.BRAND = Object.assign(CX.BRAND || {}, {id:'tya', clientName:'TyA', name:'TyA', tagline:'Tenant TyA · Phase A controlada', theme:'tya', demoMode:false, showAITag:true, countries:['GT','HN']});
  ...
  window.CX_TYA_VISIBLE_DATA_CONTRACT = {
    ...
    currentPeriodId:CX.data.currentProjectId, currentPeriodVisits:CX.data.visitas ? CX.data.visitas().length : 0,
...
if (mode === 'apply') {
  ...
  if (before.adapterTags === 0) {
    const anchor = '<script src="core/shoppers-store.js"></script>';
    if (!html.includes(anchor)) fail('Unable to locate core/shoppers-store.js adapter anchor.', 3);
    html = html.replace(anchor, `${anchor}\n${adapterTag}`);
  }
...
const after = {
  ...
  shoppersStorePosition:afterHtml.indexOf('<script src="core/shoppers-store.js"></script>'),
...
const valid = after.payloadTags === 1 && after.bridgeTags === 1 && after.adapterTags === 1 &&
  ...
  after.shoppersStorePosition > after.dataSourcePosition && after.adapterPosition > after.shoppersStorePosition &&
  after.appBootPosition > after.adapterPosition;

const report = {
  schemaVersion:'1.3.0', reportId:'phase-a-source-safe-binding-build-r15f-r17', generatedAt:new Date().toISOString(),
  ...
  validation:{
    ...
    adapterAfterDataSource:after.adapterPosition > after.dataSourcePosition,
    adapterAfterShopperStore:after.adapterPosition > after.shoppersStorePosition,
    adapterBeforeAppBoot:after.adapterPosition < after.appBootPosition,
    protectedReferencesNotNormalizedAsProfiles:adapterCode.includes("dataLevel:'protected_reference'") && adapterCode.includes('visitas:undefined') && adapterCode.includes('perfilCompleto:false'),
    ...
    pass:valid && adapterCode.includes('visitas:undefined') && adapterCode.includes('perfilCompleto:false') && !adapterCode.includes('HR viva source-safe') && !adapterCode.includes('rating: 4.3') && adapterCode.includes('runtimeSyncActive:false')
  },
  gates:{frontendModulesModified:false,coreFilesModified:false,providerCalls:false,firestoreWrites:false,authWrites:false,imports:false,deploy:false,production:false,runtimeSyncActive:false,shopperRatingInvented:false,shopperStatusInvented:false,shopperProfileCompletenessInvented:false}
};
...
fs.writeFileSync(path.join(reportDir,'source-safe-binding-r15f-report.md'), [
  ...
  `Adapter after shopper-store: ${report.validation.adapterAfterShopperStore}`,
  `Adapter before app boot: ${report.validation.adapterBeforeAppBoot}`,
  `Protected references normalized as profiles: false`,
  `Runtime sync active: false`,`Invented shopper rating/status/profile completeness: false`,'',
