...
const outDir = process.env.CXORBIA_TYA_READINESS_OUT || path.join(repoRoot, 'tmp', 'tya-readiness-consolidated');
...
if(data.sanitized?.counts?.sensitiveFieldOccurrencesRemoved > 0){
  blockers.push({ id: 'PII_REVIEW', severity: 'review', reason: 'Sanitized candidate removed sensitive fields; audit must be reviewed.', next: 'Review removedSensitiveFields.audit.json locally.' });
}
if(data.shopperIdentity?.policy?.writeRequiresManualReview){
  blockers.push({ id: 'SHOPPER_REVIEW', severity: 'review', reason: 'Some shopper identity cases require review.', next: 'Review shopper duplicate and reference review files.' });
}
...
const report = {
  ...
  mode: 'readiness-consolidated-no-runtime-actions',
  ...
  next: [
    ...
    'Review sensitive-field audit locally.',
    'Review shopper identity low-confidence cases.',
...
fs.writeFileSync(path.join(outDir, 'readinessConsolidated.json'), JSON.stringify(report, null, 2), 'utf8');
...
fs.writeFileSync(path.join(outDir, 'readinessConsolidated.md'), md, 'utf8');
console.log(md);
console.log('');
