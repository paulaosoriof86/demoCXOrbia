...
const outDir = path.join(repoRoot, 'tmp', 'tya-readiness-consolidated-v5');
...
const files = {
  ...
  candidateDecision: path.join(repoRoot, 'tmp', 'tya-operative-candidates-dev-policy-decision', 'operativeCandidatesDevPolicyDecision.json'),
...
const candidateDecision = data.candidateDecision?.decision || 'missing';
if (candidateDecision === 'candidate_only_allowed_for_dev_staging_no_final_records') {
  notes.push({ id: 'CANDIDATE_REVIEW', status: 'candidate_only_dev_policy', reason: data.candidateDecision?.reason || 'Candidate only policy.', next: 'Do not create final operational or finance records.' });
} else if (candidateDecision !== 'missing') {
  blockers.push({ id: 'CANDIDATE_REVIEW', severity: 'review', reason: 'Operative candidates still need review.', next: 'Review candidate decision.' });
}
...
fs.writeFileSync(path.join(outDir, 'readinessConsolidatedV5.json'), JSON.stringify(report, null, 2), 'utf8');
...
fs.writeFileSync(path.join(outDir, 'readinessConsolidatedV5.md'), md, 'utf8');
console.log(md);
