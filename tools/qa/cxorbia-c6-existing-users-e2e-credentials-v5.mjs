...
const technicalFields=['sourceKey','shopperId','legacyId','externalId','externalShopperId','sourceId','legacyShopperId','canonicalShopperId','plannedShopperId','liveShopperId','profileShopperId'];
const technicalArrays=['legacyLiveShopperIds','legacyShopperIds','identityAliases','sourceShopperIds'];

function stageFail(message){
  const safe=String(message||'unknown').replace(/[^A-Z0-9_:-]/gi,'_').slice(0,170);
  if(process.env.OUT_DIR){
    try{fs.mkdirSync(process.env.OUT_DIR,{recursive:true});fs.writeFileSync(path.join(process.env.OUT_DIR,'stage'),'select_existing_credentials_v5__'+safe+'\n','utf8');}catch{}
  }
...
function initialPassword(profile){
  const first=firstNameOf(profile);return first?first.charAt(0).toUpperCase()+first.slice(1)+'123*':'';
}
...
function shopperPasswordCandidates(...profiles){
  const values=new Set();
...
function technicalKeys(id,data){
  const values=new Set();
  const add=value=>{const v=text(value);if(v)values.add(v);};
  add(id);
  for(const field of technicalFields)add(data?.[field]);
  for(const field of technicalArrays)for(const value of list(data?.[field]))add(value);
  return [...values];
}
...
const plannedToCanonical=new Map(),canonicalVisitCounts=new Map();
let resolvedHistoryVisits=0;
for(const row of Array.isArray(crosswalk.crosswalk)?crosswalk.crosswalk:[]){
  ...
  const prior=plannedToCanonical.get(plannedId);if(prior&&prior!==canonicalId)stageFail('VISIT_CROSSWALK_PLANNED_ID_CONFLICT');
  ...
  resolvedHistoryVisits+=count;
}
if(plannedToCanonical.size!==Number(crosswalk.counts?.resolvedRefs||0)||Number(crosswalk.counts?.conflictRefs||0)!==0)stageFail('VISIT_CROSSWALK_COUNTS_INVALID');
if(resolvedHistoryVisits<Number(crosswalk.counts?.visitMatchesUniqueShopper||0)||resolvedHistoryVisits>Number(crosswalk.counts?.visitsWithShopperRef||0))stageFail('VISIT_CROSSWALK_HISTORY_RANGE_INVALID');

const canonicalKeyOwners=new Map();
const ambiguousCanonicalKeys=new Set();
for(const [canonicalId] of canonicalVisitCounts){
  const data=shopperById.get(canonicalId);if(!data)continue;
  for(const key of technicalKeys(canonicalId,data)){
    const prior=canonicalKeyOwners.get(key);
    if(prior&&prior!==canonicalId){ambiguousCanonicalKeys.add(key);canonicalKeyOwners.delete(key);}
    else if(!ambiguousCanonicalKeys.has(key))canonicalKeyOwners.set(key,canonicalId);
  }
}
function exactCanonicalFromClaim(claimShopperId,claimProfile){
  const candidates=new Set();
  const add=id=>{const value=text(id);if(value&&canonicalVisitCounts.has(value))candidates.add(value);};
  add(claimShopperId);
  add(plannedToCanonical.get(claimShopperId));
  for(const key of technicalKeys(claimShopperId,claimProfile||{})){
    add(plannedToCanonical.get(key));
    if(!ambiguousCanonicalKeys.has(key))add(canonicalKeyOwners.get(key));
  }
  return candidates.size===1?[...candidates][0]:'';
}

let shopper=null;
let shopperRecords=0,authUsersFound=0,claimProfilesFound=0,canonicalTargets=0,hashMatches=0,passwordMatches=0;
for(const record of Array.isArray(bundle.records)?bundle.records:[]){
  ...
  const claimProfile=shopperById.get(claimShopperId);
  if(!claimProfile)continue;
  claimProfilesFound++;
  const canonicalShopperId=exactCanonicalFromClaim(claimShopperId,claimProfile);
  if(!canonicalShopperId)continue;
  canonicalTargets++;
  const canonicalProfile=shopperById.get(canonicalShopperId)||{};
  for(const candidate of shopperPasswordCandidates(claimProfile,canonicalProfile)){
    if(sha256Hex(candidate)!==hash)continue;
    hashMatches++;
...
if(!shopper)stageFail(`HOLD_SHOPPER_R${shopperRecords}_U${authUsersFound}_D${claimProfilesFound}_C${canonicalTargets}_H${hashMatches}_S${passwordMatches}_K${canonicalKeyOwners.size}_X${ambiguousCanonicalKeys.size}`);

fs.mkdirSync(path.dirname(outPath),{recursive:true});
fs.writeFileSync(outPath,JSON.stringify({schemaVersion:'cxorbia.c6.e2e-private-credentials.v6',staff,shopper},null,2)+'\n',{encoding:'utf8',mode:0o600});
console.log(JSON.stringify({decision:'PASS_C6_EXISTING_E2E_CREDENTIAL_SELECTION_V5',staffRole:staff.role,shopperRole:shopper.role,shopperOwnVisits:shopper.expectedOwnVisits,claimResolvedThroughExactTechnicalBridge:shopper.shopperId!==shopper.canonicalShopperId,visitCrosswalkResolved:Number(crosswalk.counts?.resolvedRefs||0),resolvedShopperHistoryVisits:resolvedHistoryVisits,visitMatchesExactEvidence:Number(crosswalk.counts?.visitMatchesUniqueShopper||0),technicalCanonicalKeys:canonicalKeyOwners.size,ambiguousTechnicalKeys:ambiguousCanonicalKeys.size,authWrites:0,passwordChanges:0,valuesExported:false}));
