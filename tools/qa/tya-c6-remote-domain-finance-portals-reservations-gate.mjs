...
try{
  ...
  const staff=await staffSession.page.evaluate(()=>{
    ...
    return {
      ...
      modules:{cliente:typeof window.CX?.modules?.cliente==='function',shopper:typeof window.CX?.modules?.miperfil==='function',finance:typeof window.CX?.modules?.finanzas==='function',reservations:typeof window.CX?.modules?.reservas==='function'}
    };
  });
  ...
  const evidence={
    schemaVersion:'cxorbia.c6.remote-domain-finance-portals-reservations.v1',
