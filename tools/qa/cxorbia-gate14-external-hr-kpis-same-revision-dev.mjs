...
const external=await jsonFetch(`${HOSTING_URL}/api/tya/cinepolis/hr-live?format=json&fresh=1&gate14=${Date.now()}`);
ensure(external.response.ok&&external.body,'ENVIRONMENT_FAILURE',{blocker:`GATE14_EXTERNAL_HR_HTTP_${external.response.status}`});
const snapshot=external.body?.snapshot||external.body?.data||external.body;
const runtime={...(external.body?._runtime||{}),...(snapshot?._runtime||{})};
...
const rawVisits=arr(snapshot.visits);
const rawShopperIds=new Set(rawVisits.map(v=>str(v.shopperId)).filter(Boolean));
...
try{
  ...
  result=await page.evaluate(({rootProjectId,externalRevision})=>{
    const C=window.CX,D=C?.data,auth=C?.backendAuth?.context?.()||{};
    if(!C||!D)return {ok:false,blocker:'GATE14_CX_DATA_UNAVAILABLE'};
    if(!(auth.authenticated===true&&auth.provider==='firebase'&&['super','admin'].includes(String(auth.role||''))))return {ok:false,blocker:'GATE14_FIREBASE_ADMIN_CONTEXT_INVALID'};
    const browserRevision=String(D.previewMeta?.sourceRevision||'');
    if(browserRevision!==externalRevision)return {ok:false,blocker:'GATE14_BROWSER_HR_REVISION_MISMATCH',browserRevision,externalRevision};
    ...
    const rootForVisit=v=>{
      ...
      const p=projects.find(x=>String(x?.id||'')===pid)||null;
    ...
    const target=shoppers.find(s=>s&&s.__canonicalIdentityOverlay===true&&level(s)!=='protected_reference'&&String(s.nombre||'').trim()&&scopedVisits(s.id).length>0)||null;
    if(!target)return {ok:false,blocker:'GATE14_EXACT_PROFILE_WITH_HISTORY_MISSING'};
    C.router?.nav?.('shoppers');
    return {ok:true,targetId:String(target.id),targetName:String(target.nombre||''),browserRevision,root,browserScopedCount:scopedVisits(target.id).length};
  },{rootProjectId,externalRevision});
  ensure(result?.ok,result?.blocker==='GATE14_BROWSER_HR_REVISION_MISMATCH'?'ENVIRONMENT_FAILURE':'PROVIDER_FAILURE',{blocker:result?.blocker||'GATE14_BROWSER_TARGET_SELECTION_FAILED',browserRevision:result?.browserRevision||null,externalRevision});
  ensure(rawShopperIds.has(result.targetId),'MAPPING_FAILURE',{blocker:'GATE14_EXTERNAL_HR_TARGET_ID_NOT_EXACT',shopperFingerprint:fp(result.targetId),externalRevision});

  const externalVisits=rawVisits.filter(v=>str(v.shopperId)===result.targetId);
  const expected=stats(externalVisits);
  ...
  ensure(Number(result.browserScopedCount)===expected.total,'FUNCTIONAL_DEFECT',{blocker:'GATE14_BROWSER_HISTORY_COUNT_DIFFERS_FROM_EXTERNAL_HR',browserCount:result.browserScopedCount,externalCount:expected.total,externalRevision,shopperFingerprint:fp(result.targetId)});
  ...
  const actual=await page.evaluate(()=>{
    ...
    return {total:numberOf('all'),realizadas:numberOf('done'),liquidadas:numberOf('liq'),enCurso:numberOf('curso'),browserRevision:String(window.CX?.data?.previewMeta?.sourceRevision||''),authorityApplied:window.CX_PROTECTED_AUTH_HR_AUTHORITY?.applied===true};
  });
  ...
  ensure(kpisExact,'FUNCTIONAL_DEFECT',{blocker:'GATE14_KPIS_DO_NOT_MATCH_SAME_EXTERNAL_HR_REVISION',externalRevision,expected,actual,shopperFingerprint:fp(result.targetId)});
  ensure(actual.authorityApplied===true,'AUTH_FAILURE',{blocker:'GATE14_HR_AUTHORITY_NOT_APPLIED'});

  const external2=await jsonFetch(`${HOSTING_URL}/api/tya/cinepolis/hr-live?format=json&fresh=1&gate14_readback=${Date.now()}`);
  const snapshot2=external2.body?.snapshot||external2.body?.data||external2.body;
  const runtime2={...(external2.body?._runtime||{}),...(snapshot2?._runtime||{})};
  const readbackRevision=str(runtime2.revision);
  ensure(external2.response.ok&&readbackRevision===externalRevision,'ENVIRONMENT_FAILURE',{blocker:'GATE14_EXTERNAL_HR_REVISION_CHANGED_DURING_CERTIFICATION',startRevision:externalRevision,endRevision:readbackRevision||null});

  result={shopperFingerprint:fp(result.targetId),externalRevision,browserRevision:actual.browserRevision,readbackRevision,expected,actual:{total:actual.total,realizadas:actual.realizadas,liquidadas:actual.liquidadas,enCurso:actual.enCurso},kpisExact:true,authorityApplied:true,externalVisitCount:externalVisits.length};
  await ctx.close();
}finally{browserToken='';await browser.close();}

finish('PASS_GATE14_KPIS_SAME_EXTERNAL_HR_REVISION',{sourceSha:process.env.SOURCE_SHA||null,lockedGate13RunId:Number(gate13.lockedRunId)||null,tenantId,projectId:rootProjectId,periodId,shopperFingerprint:result.shopperFingerprint,externalRevision:result.externalRevision,browserRevision:result.browserRevision,readbackRevision:result.readbackRevision,sameRevision:true,externalVisitCount:result.externalVisitCount,kpiExpected:result.expected,kpiActual:result.actual,kpisExact:result.kpisExact,authorityApplied:result.authorityApplied,externalAuthority:'hr-live-runtime',comparisonIndependentFromBrowserReadModel:true,pageErrors},0);
