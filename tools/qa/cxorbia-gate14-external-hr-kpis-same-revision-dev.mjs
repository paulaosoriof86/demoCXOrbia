...
try{
  ...
  const pendingOverlayVisibility=Math.max(0,Number(result.browserScopedCount)-Number(expected.total));
  ...
  ensure(kpisExact,'FUNCTIONAL_DEFECT',{blocker:'GATE14_KPIS_DO_NOT_MATCH_SAME_EXTERNAL_HR_REVISION',externalRevision,expected,actual,shopperFingerprint:fp(result.targetId),exactSourceIdCount:exactSourceIds.size,browserHistoryCount:result.browserScopedCount,pendingOverlayVisibility});
