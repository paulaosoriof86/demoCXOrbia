...
check('live_watcher_never_raw_applies_after_protected_authority',liveWatcher.includes("if(authenticatedHumanRuntime&&canonicalProtectedAuthorityReady())")&&liveWatcher.includes("authority:'protected_auth_hr_reconcile'"),'authenticated human watcher must delegate revision changes to protected HR authority and never raw-apply over composed Firestore profiles');
