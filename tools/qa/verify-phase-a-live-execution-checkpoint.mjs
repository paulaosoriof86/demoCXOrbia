...
const iteration2Gate=path.join(repo,'tools/qa/verify-root-cause-correction-iteration2.mjs');
const read=p=>fs.readFileSync(p,'utf8').replace(/^\uFEFF/,'');
const ensure=(v,c)=>{if(!v)throw new Error(c);};
for(const p of [currentCheckpoint,sourceIndex,forensic,durablePlan,handoffLock,v2Contract,v2Request,iteration1Gate,iteration2Gate])ensure(fs.existsSync(p),`CURRENT_AUTHORITY_MISSING:${path.relative(repo,p)}`);
...
ensure(idx.includes('ROOT_CAUSE_CORRECTION_EXECUTION')||idx.includes('ITERACION_1')||idx.includes('ITERACION_3'),'SOURCE_INDEX_CORRECTION_PATH_MISSING');
...
const iteration2Pass=cp.includes('ITERATION_2_CANONICAL_PERSISTENCE_PASS')||cp.includes('ITERACION_2_CANONICAL_PERSISTENCE_PASS');
const iteration1Pass=iteration2Pass||cp.includes('ITERATION_1_SOURCE_ONLY_PASS')||cp.includes('ITERACION_1_SOURCE_ONLY_PASS');
let progress={goLiveCorrectionCompletedPct:0,goLiveCorrectionRemainingPct:100,iteration:'0/5',basis:'durable-root-cause-go-live-plan'};
if(iteration1Pass)progress={goLiveCorrectionCompletedPct:15,goLiveCorrectionRemainingPct:85,iteration:'1/5',basis:'durable-root-cause-go-live-plan'};
if(iteration2Pass)progress={goLiveCorrectionCompletedPct:35,goLiveCorrectionRemainingPct:65,iteration:'2/5',basis:'durable-root-cause-go-live-plan'};
if(iteration1Pass)ensure(cp.includes('PASS_ROOT_CAUSE_CORRECTION_ITERATION1_SOURCE_ONLY'),'ITERATION_1_PASS_EVIDENCE_MARKER_MISSING');
if(iteration2Pass){
  ensure(cp.includes('PASS_ROOT_CAUSE_CORRECTION_ITERATION2_CANONICAL_PERSISTENCE'),'ITERATION_2_PASS_EVIDENCE_MARKER_MISSING');
  ensure(cp.includes('ITERACION_3_DEV_AUTH_FIRESTORE_SHOPPER_PERSISTENCE'),'ITERATION_3_NEXT_MISSING');
  ensure(/35%\s*(?:completado|completed)/i.test(cp),'ITERATION_2_PROGRESS_35_MISSING');
}else if(iteration1Pass)ensure(cp.includes('ITERACION_2_CANONICAL_PERSISTENCE_AND_TRANSVERSAL_REGRESSION'),'ITERATION_2_NEXT_MISSING');

const result={ok:true,decision:'PASS_PHASE_A_CURRENT_OPERATIONAL_CHECKPOINT_FORENSIC_PLAN',authority:'app/docs/CHECKPOINT-OPERATIVO-CXORBIA-TYA-VIGENTE.md',progress,priorTechnicalProgress:{retained:true,authoritativeForProductionReadiness:false},exactWriteV2:{preserved:true,consumed:r.consumed===true,decision:r.decision||null},locks:{sameCandidate:true,noGeneralRediagnosis:true,noAuthRebuild:true,noNewBranchOrPr:true},safety:{authWrites:false,firestoreWrites:false,hrWrites:false,rulesWrites:false,storageWrites:false,deploy:false,merge:false,production:false}};
