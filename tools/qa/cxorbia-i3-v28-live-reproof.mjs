...
async function signed(member,kind){
  ...
  for(let i=0;i<5;i++){
    try{await page.evaluate(async t=>{await firebase.auth().setPersistence(firebase.auth.Auth.Persistence.LOCAL);await firebase.auth().signInWithCustomToken(t);},token);}catch(e){if(!/Execution context was destroyed|navigation/i.test(str(e?.message)))throw e;}
    await page.waitForTimeout(800);
    const uid=await page.evaluate(()=>String(firebase.auth().currentUser?.uid||'')).catch(()=> '');
    if(uid===member.id)break;
  }
