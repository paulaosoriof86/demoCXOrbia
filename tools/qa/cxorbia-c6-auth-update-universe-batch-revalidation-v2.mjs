...
const requestPath=process.argv[2]||'backend/config/c6-auth-update-universe-batch-revalidation-request-v4.json';
...
const exportDir=path.resolve(process.env.CXORBIA_C6_UPDATE_BATCH_OUT||path.join(root,'.tmp/c6-auth-update-universe-batch-v2'));
...
function deriveSystemicRiskRows(plan){
  const groups=new Map();
  for(const row of plan.rows){const key=text(row.baseLoginFp);if(!key)continue;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(row);}
  const updateFps=new Set(plan.rows.filter(r=>r.primary==='UPDATE_AUTH').map(r=>r.profileFp));
  const risk=new Set();
  for(const row of plan.rows){
    if(!updateFps.has(row.profileFp)||row.suffixApplied!==true||!row.baseLoginFp)continue;
    const group=groups.get(row.baseLoginFp)||[];
    if(group.length!==2)continue;
    const peer=group.find(x=>x.profileFp!==row.profileFp);
    if(peer&&peer.suffixApplied!==true)risk.add(row.profileFp);
  }
  return risk;
}
...
function rebuild(plan,classification,authUsersBefore){
  ...
  next.sourceLineage={...(next.sourceLineage||{}),updateUniverseBatchRevalidation:'C6_AUTH_UPDATE_UNIVERSE_BATCH_REVALIDATION_V2',priorPlanDigest:INPUT_DIGEST};
  ...
  for(const row of next.rows){
    ...
    row.batchRevalidation={
      version:'v2',priorPrimary:'UPDATE_AUTH',candidateCount:c.candidateCount,candidateClass:c.candidateClass,
      systemicSuffixRisk:c.systemicSuffixRisk,crossRowAssociation:c.crossRowAssociation,unresolved:c.unresolved,
...
function selfTest(){
  const sample={rows:[
    {profileFp:'peer0',primary:'NO_OP',baseLoginFp:'b0',suffixApplied:false,changes:{email:false,password:false,claims:false}},
    {profileFp:'p0',primary:'UPDATE_AUTH',baseLoginFp:'b0',suffixApplied:true,changes:{email:true,password:false,claims:true}},
    {profileFp:'p1',primary:'UPDATE_AUTH',baseLoginFp:'b1',suffixApplied:false,changes:{email:true,password:true,claims:false}},
    {profileFp:'p2',primary:'UPDATE_AUTH',baseLoginFp:'b2',suffixApplied:true,changes:{email:true,password:false,claims:true}}
  ],sourceLineage:{}};
  const risk=deriveSystemicRiskRows(sample);ensure(risk.has('p0')&&!risk.has('p1'),'SELFTEST_RISK_DERIVATION');
  const cls=[
    ...
    {profileFp:'p2',candidateCount:2,candidateClass:'>1',systemicSuffixRisk:false,crossRowAssociation:true,unresolved:true,unresolvedReasons:['MULTI_CANDIDATE'],emailDiff:null,claimsDiff:null}
  ];
  const n=rebuild(sample,cls,10);ensure(n.rows.find(r=>r.profileFp==='p0').primary==='CREATE_AUTH','SELFTEST_ZERO');ensure(n.rows.find(r=>r.profileFp==='p1').primary==='UPDATE_AUTH','SELFTEST_ONE');ensure(n.rows.find(r=>r.profileFp==='p2').primary==='HOLD','SELFTEST_MULTI');
  console.log('PASS_C6_AUTH_UPDATE_UNIVERSE_BATCH_V2_SELFTEST');
}
...
async function main(){
  ...
  const riskRows=deriveSystemicRiskRows(plan);ensure(riskRows.size===EXPECTED_SYSTEMIC_RISK_ROWS,'DERIVED_RISK_SCOPE_NOT_36');
  ...
  ensure(classification.filter(x=>x.systemicSuffixRisk).length===EXPECTED_SYSTEMIC_RISK_ROWS,'RISK_COVERAGE_NOT_36');
  ...
  const evidence={
    schemaVersion:'cxorbia.c6.auth-update-universe-batch-revalidation.evidence.v2',generatedAt:new Date().toISOString(),decision,requestId:request.requestId,
    ...
    systemicRisk:{classification:'SYSTEMIC_SUFFIXED_UPDATE_SHARED_BASELOGIN_ALIAS_RISK',expectedRows:EXPECTED_SYSTEMIC_RISK_ROWS,derivedFromCanonicalPlanStructure:true,coveredByFullUpdateUniverse:true},
    classificationSummary:{rows:45,riskRows:EXPECTED_SYSTEMIC_RISK_ROWS,all45UpdateRowsClassified:true,...counts},classification,
...
if(process.argv.includes('--self-test'))selfTest();else main().catch(error=>{
  ...
  fs.writeFileSync(path.join(exportDir,'decision.txt'),decision+'\n','utf8');
  fs.writeFileSync(path.join(exportDir,'batch-classification-source-safe.json'),JSON.stringify({schemaVersion:'cxorbia.c6.auth-update-universe-batch-revalidation.evidence.v2',decision,error:String(error?.message||error).replace(/[^A-Za-z0-9_.:,/+-]+/g,'_').slice(0,400),safety:{providerWrites:0,authWrites:0,firestoreWrites:0,hrWrites:0,merge:false,production:false}},null,2)+'\n','utf8');
