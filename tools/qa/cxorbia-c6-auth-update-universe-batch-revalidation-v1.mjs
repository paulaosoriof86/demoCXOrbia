...
const requestPath=process.argv[2]||'backend/config/c6-auth-update-universe-batch-revalidation-request-v1.json';
...
const exportDir=path.resolve(process.env.CXORBIA_C6_UPDATE_BATCH_OUT||path.join(root,'.tmp/c6-auth-update-universe-batch'));
...
function rebuild(plan,classification,authUsersBefore){
  const next=structuredClone(plan);next.schemaVersion='cxorbia.c6.shopper-auth-final-plan.v4';next.generatedAt=new Date().toISOString();
  next.sourceLineage={...(next.sourceLineage||{}),updateUniverseBatchRevalidation:'C6_AUTH_UPDATE_UNIVERSE_BATCH_REVALIDATION_V1',priorPlanDigest:INPUT_DIGEST};
  ...
  for(const row of next.rows){
    ...
    row.batchRevalidation={
      version:'v1',
      priorPrimary:'UPDATE_AUTH',
      candidateCount:c.candidateCount,
      candidateClass:c.candidateClass,
      systemicSuffixRisk:c.systemicSuffixRisk,
      crossRowAssociation:c.crossRowAssociation,
  ...
  const summary=countPlan(next.rows);const digest=rowsDigest(next.rows);const holdZero=summary.operationCounts.HOLD===0;
  next.plan={
    rows:340,uniqueRows:340,operationCounts:summary.operationCounts,subchangeCounts:summary.subchangeCounts,
    sourceSafeRowsDigestSha256:digest,onePrimaryOperationPerProfile:true,holdZero,
    expectedAuthUsersBefore:authUsersBefore,
    expectedAuthUsersAfterIfFullyExecutable:authUsersBefore+summary.operationCounts.CREATE_AUTH,
    globalExistingPrincipalUniquenessRequired:true,
    executable:false,
    executableReason:holdZero?'BATCH_REVALIDATION_COMPLETE_PENDING_SEPARATE_PREWRITE_AUTH_AUTHORIZATION':'BATCH_REVALIDATION_HAS_UNRESOLVED_HOLD'
  };
...
function selfTest(){
  ...
  const cls=[
    {profileFp:'p0',candidateCount:0,candidateClass:'0',systemicSuffixRisk:true,crossRowAssociation:false,unresolved:false,unresolvedReasons:[],emailDiff:null,claimsDiff:null},
    {profileFp:'p1',candidateCount:1,candidateClass:'1',systemicSuffixRisk:false,crossRowAssociation:false,unresolved:false,unresolvedReasons:[],emailDiff:false,claimsDiff:false},
    {profileFp:'p2',candidateCount:2,candidateClass:'>1',systemicSuffixRisk:true,crossRowAssociation:true,unresolved:true,unresolvedReasons:['MULTI_CANDIDATE'],emailDiff:null,claimsDiff:null}
  ];
  ...
  console.log('PASS_C6_AUTH_UPDATE_UNIVERSE_BATCH_SELFTEST');
}

async function main(){
  fs.mkdirSync(exportDir,{recursive:true});
  const request=JSON.parse(fs.readFileSync(requestPath,'utf8'));const contract=JSON.parse(fs.readFileSync(request.contract,'utf8'));const plan=JSON.parse(fs.readFileSync(planPath,'utf8'));const riskEvidence=JSON.parse(fs.readFileSync(riskEvidencePath,'utf8'));
  ...
  const updateRows=plan.rows.filter(r=>r.primary==='UPDATE_AUTH');ensure(updateRows.length===45,'UPDATE_SCOPE_NOT_45');
  const riskRows=new Set((riskEvidence.remainingRiskRows||[]).map(r=>r.profileFp));ensure(riskEvidence.systemicFinding?.remainingCurrentUpdateRowsInSameStructuralRiskClass===36&&riskRows.size===36,'RISK_SCOPE_NOT_36');for(const fpv of riskRows)ensure(updateRows.some(r=>r.profileFp===fpv),'RISK_ROW_NOT_UPDATE');
  ensure(serviceAccountPath&&fs.existsSync(serviceAccountPath),'SERVICE_ACCOUNT_MISSING');const serviceAccount=JSON.parse(fs.readFileSync(serviceAccountPath,'utf8'));ensure(serviceAccount.project_id===EXPECTED_FIREBASE_PROJECT,'SERVICE_ACCOUNT_PROJECT');
  ...
  const bundle=decryptCredentialBundle({serviceAccount});const tenantRef=db.collection('tenants').doc(TENANT_ID),projectRef=tenantRef.collection('projects').doc(CANONICAL_PROJECT_ID);
  ...
  const profiles=new Map(profileSnap.docs.map(doc=>[doc.id,{id:doc.id,...(doc.data()||{})}]));const profileByFp=new Map([...profiles.values()].map(profile=>[stablePlanProfileFingerprint(profile.id),profile]));ensure(profileByFp.size===340&&updateRows.every(row=>profileByFp.has(row.profileFp)),'PROFILE_SET_DRIFT');
  ...
  const protectedOwners=protectedCandidateOwners(plan);const raw=[];
  for(const planRow of updateRows){
    const profile=profileByFp.get(planRow.profileFp),credentials=credentialsByProfile.get(profile.id)||[],names=namesByProfile.get(profile.id);let targetLogin='',targetEmail='',reconstructionError=null,candidates=[];
    ...
    raw.push({planRow,profileId:profile.id,targetEmail,reconstructionError,candidates,systemicSuffixRisk:riskRows.has(planRow.profileFp)});
  }
  ensure(raw.length===45,'BATCH_CLASSIFICATION_INCOMPLETE');
  const candidateRows=new Map();for(const item of raw)for(const user of item.candidates){const cfp=stableAuthCandidateFingerprint(user.uid);if(!candidateRows.has(cfp))candidateRows.set(cfp,new Set());candidateRows.get(cfp).add(item.planRow.profileFp);}
  const classification=[];
  for(const item of raw){
    const candidateFps=item.candidates.map(u=>stableAuthCandidateFingerprint(u.uid));const candidateCount=item.reconstructionError?0:item.candidates.length;const candidateClass=item.reconstructionError?'UNRESOLVED':candidateCount===0?'0':candidateCount===1?'1':'>1';const unresolvedReasons=[];
    ...
    if(!item.reconstructionError&&candidateCount===1){
      const user=item.candidates[0],cfp=candidateFps[0],rowOwners=[...(candidateRows.get(cfp)||[])];if(rowOwners.length>1){crossRowAssociation=true;unresolvedReasons.push('BATCH_CROSS_ROW_CANDIDATE_ALIAS');}
      const claimSid=text(user.customClaims?.shopperId);if(claimSid&&profiles.has(claimSid)){claimOwnerProfileFp=stablePlanProfileFingerprint(claimSid);if(claimOwnerProfileFp!==item.planRow.profileFp){crossRowAssociation=true;unresolvedReasons.push('CURRENT_CLAIM_POINTS_TO_OTHER_PROFILE');}}
      const protectedOwner=protectedOwners.get(cfp);if(protectedOwner&&protectedOwner.profileFp!==item.planRow.profileFp){protectedOwnerProfileFp=protectedOwner.profileFp;crossRowAssociation=true;unresolvedReasons.push('PROTECTED_SOURCE_OWNER_CONFLICT');}
      const owners=authByEmail.get(norm(item.targetEmail))||[];if(owners.some(owner=>owner.uid!==user.uid)){targetEmailCollision=true;unresolvedReasons.push('TARGET_EMAIL_COLLISION');}
    ...
    classification.push({profileFp:item.planRow.profileFp,currentPrimary:'UPDATE_AUTH',systemicSuffixRisk:item.systemicSuffixRisk,candidateCount,candidateClass,candidateFingerprints:candidateFps,crossRowAssociation,claimOwnerProfileFp,protectedOwnerProfileFp,targetEmailCollision,passwordProviderMissing,emailDiff,claimsDiff,currentPasswordChangeFlag:Boolean(item.planRow.changes?.password),unresolved,unresolvedReasons});
  }
  ensure(classification.length===45,'CLASSIFICATION_OUTPUT_NOT_45');
  ...
  const rebuilt=rebuild(plan,classification,authUsers.length);const rebuiltCounts=rebuilt.plan.operationCounts;const decision=counts.unresolved>0?'STOP_RETRY_C6_AUTH_UPDATE_UNIVERSE_BATCH_REVALIDATION_UNRESOLVED':'PASS_C6_AUTH_UPDATE_UNIVERSE_BATCH_REVALIDATION_PLAN_V4';
  const evidence={
    schemaVersion:'cxorbia.c6.auth-update-universe-batch-revalidation.evidence.v1',generatedAt:new Date().toISOString(),decision,requestId:request.requestId,inputPlan:{rows:340,updateRows:45,digest:INPUT_DIGEST},providerRead:{attempts:1,authDirectoryPages:1,authUsers:authUsers.length,shopperDocuments:profileSnap.docs.length,hrDocuments:hrSnap.docs.length,visitDocuments:visitSnap.docs.length,certificationDocuments:certSnap.docs.length,liquidationDocuments:liqSnap.docs.length,credentialRecords:credentialRecords.length,credentialsMapped,credentialsUnmapped},classificationSummary:{rows:45,riskRows:riskRows.size,...counts},classification,rebuiltPlan:{rows:340,uniqueRows:340,operationCounts:rebuilt.plan.operationCounts,subchangeCounts:rebuilt.plan.subchangeCounts,expectedAuthUsersBefore:rebuilt.plan.expectedAuthUsersBefore,expectedAuthUsersAfterIfFullyExecutable:rebuilt.plan.expectedAuthUsersAfterIfFullyExecutable,digest:rebuilt.plan.sourceSafeRowsDigestSha256,holdZero:rebuilt.plan.holdZero,executable:false,executableReason:rebuilt.plan.executableReason},passwordPolicy:{materialInspected:false,signInProbeUsed:false,flagsForRowsRemainingUniqueUpdate:'PRESERVED_FROM_V3',flagsClearedWhenReclassified:true},safety:{providerWrites:0,authWrites:0,firestoreWrites:0,hrWrites:0,visitsWrites:0,certificationsWrites:0,liquidationsWrites:0,rulesWrites:0,storageWrites:0,cloudBuild:0,cloudRun:0,hosting:0,make:0,gemini:0,payments:0,merge:false,production:false,rawUidExported:false,rawEmailExported:false,rawPasswordExported:false,rawClaimsExported:false,passwordMaterialExported:false}}
  fs.writeFileSync(path.join(exportDir,'batch-classification-source-safe.json'),JSON.stringify(evidence,null,2)+'\n','utf8');fs.writeFileSync(path.join(exportDir,'final-plan-v4-source-safe.json'),JSON.stringify(rebuilt,null,2)+'\n','utf8');fs.writeFileSync(path.join(exportDir,'decision.txt'),decision+'\n','utf8');console.log(decision);console.log(JSON.stringify({classificationSummary:evidence.classificationSummary,rebuiltPlan:evidence.rebuiltPlan}));
}

if(process.argv.includes('--self-test'))selfTest();else main().catch(error=>{fs.mkdirSync(exportDir,{recursive:true});const decision='STOP_RETRY_C6_AUTH_UPDATE_UNIVERSE_BATCH_REVALIDATION_TECHNICAL';fs.writeFileSync(path.join(exportDir,'decision.txt'),decision+'\n','utf8');fs.writeFileSync(path.join(exportDir,'batch-classification-source-safe.json'),JSON.stringify({schemaVersion:'cxorbia.c6.auth-update-universe-batch-revalidation.evidence.v1',decision,error:String(error?.message||error).replace(/[^A-Za-z0-9_.:,/+-]+/g,'_').slice(0,400),safety:{providerWrites:0,authWrites:0,firestoreWrites:0,hrWrites:0,merge:false,production:false}},null,2)+'\n','utf8');console.error(decision,error?.message||error);process.exitCode=2;});
