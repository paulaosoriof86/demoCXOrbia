...
const clean=v=>String(v??'').replace(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+/g,'REDACTED_EMAIL').replace(/([?&](?:token|key|password|secret|auth|code)=)[^&\s]+/gi,'$1REDACTED').slice(0,2600);
...
fs.mkdirSync(outDir,{recursive:true});fs.mkdirSync(path.join(outDir,'screenshots'),{recursive:true});
const persist=v=>fs.writeFileSync(outputFile,JSON.stringify(v,null,2)+'\n','utf8');

async function state(page,label){
  return page.evaluate(label=>{
    const c=window.CX?.backendAuth?.context?.()||{},a=window.CX_PROTECTED_AUTH_HR_AUTHORITY||{},h=window.CX_C6_LIVE_USER_ADMIN_FRONTEND_HANDOFF||{},d=window.CX?.data||{},ds=window.CX?.dataSource||{},lock=window.CX?.SOURCE_LOCK||window.CX_SOURCE_LOCK||null;
    ...
    const keys=visits.map(key),periodKeys=Array.isArray(d.projects)?d.projects.map(p=>String(p?.periodKey||p?.key||p?.id||'').trim()).filter(Boolean):[];
    const app=document.getElementById('app'),rail=document.getElementById('rail'),view=document.getElementById('view');
    let protectedEmpty=null;try{protectedEmpty=typeof window.CX_C4_EMPTY_SHELL_GUARD?.isProtectedEmpty==='function'?window.CX_C4_EMPTY_SHELL_GUARD.isProtectedEmpty():null;}catch(e){protectedEmpty='error';}
    let blocked=null;try{blocked=typeof ds.isBlocked==='function'?ds.isBlocked():null;}catch(e){blocked='error';}
    const appText=String(app?.innerText||'').replace(/\s+/g,' ').trim();
    return {
      label,role:c.role||null,namespace:c.authNamespace||null,tenantId:c.tenantId||null,projectIds:Array.isArray(c.projectIds)?c.projectIds.slice():[],
      membershipVerified:window.CX?.session?.user?.membershipVerified===true,periods:Array.isArray(d.projects)?d.projects.length:-1,visits:visits.length,shoppers:Array.isArray(d.shoppers)?d.shoppers.length:-1,currentProjectId:d.currentProjectId||null,currentPeriodId:d.currentPeriodId||null,periodKeys,
      ...
      sourceRevision:d?.previewMeta?.sourceRevision||window.CX_TYA_HR_LIVE_META?.revision||null,sourceReadAt:d?.previewMeta?.sourceReadAt||window.CX_TYA_HR_LIVE_META?.sourceReadAt||null,sourceMode:d.sourceMode||null,dataMode:ds.mode||null,dataStatus:ds.status||null,dataSourceRef:ds.sourceRef||null,runtimeReadActive:ds.runtimeReadActive===true,runtimeSyncActive:ds.runtimeSyncActive===true,dataBlocked:blocked,dataBlockers:Array.isArray(ds.blockers)?ds.blockers.slice():[],
      ...
      appOn:app?.classList.contains('on')===true,loginHidden:document.getElementById('login')?.classList.contains('hidden')===true,currentView:window.CX?.session?.view||null,
      visitKeys:keys,missingVisitKeys:keys.filter(x=>!x).length,uniqueVisitKeys:new Set(keys.filter(Boolean)).size,
      ...
      shell:{appExists:!!app,railExists:!!rail,viewExists:!!view,railChildren:rail?.children?.length??-1,navIds:[...document.querySelectorAll('.nav-i[id]')].map(x=>x.id),navCount:document.querySelectorAll('.nav-i[id]').length,navDashboardExists:!!document.getElementById('nav-dashboard'),navDashboardVisible:!!document.getElementById('nav-dashboard')?.getClientRects().length,emptyGuardActive:window.CX_C4_EMPTY_SHELL_STATE?.active===true,emptyGuardState:window.CX_C4_EMPTY_SHELL_STATE||null,protectedEmpty,dataSourceBlock:!!document.getElementById('dsBackDemo'),dataSourceUnavailable:/Fuente de datos no disponible/i.test(appText),emptyProject:/Sin proyectos disponibles/i.test(appText),appTextPrefix:appText.slice(0,460),railTextPrefix:String(rail?.innerText||'').replace(/\s+/g,' ').trim().slice(0,460),appDirectChildren:app?[...app.children].map(x=>x.id||x.className||x.tagName):[]},
      backendLast:{source:window.CX_BACKEND_LAST_STATE?.source||null,empty:window.CX_BACKEND_LAST_STATE?.empty,reason:window.CX_BACKEND_LAST_STATE?.reason||null,counts:window.CX_BACKEND_LAST_STATE?.counts||null},
...
function validate(s,label){
  ...
  ensure(s.runtimeReadActive&&s.runtimeSyncActive===false&&s.dataStatus==='ready',label+'_READONLY_LIVE_SOURCE_INVALID');
...
async function shellDiagnosticRecovery(page,s){
  if(s.shell.navDashboardVisible)return null;
  await page.screenshot({path:path.join(outDir,'screenshots','m8-shell-failure-before-diagnostic.png'),fullPage:true});
  if(!s.shell.railExists)return {attempted:false,reason:'rail_missing_no_dom_reconstruction_allowed'};
  const result=await page.evaluate(()=>{try{const before={navCount:document.querySelectorAll('.nav-i[id]').length,view:window.CX?.session?.view||null};window.CX.router.mount();return {attempted:true,ok:true,before,after:{navCount:document.querySelectorAll('.nav-i[id]').length,navDashboardExists:!!document.getElementById('nav-dashboard'),navDashboardVisible:!!document.getElementById('nav-dashboard')?.getClientRects().length,view:window.CX?.session?.view||null}};}catch(e){return {attempted:true,ok:false,error:String(e?.message||e).slice(0,600)};}});
  await page.screenshot({path:path.join(outDir,'screenshots','m8-shell-after-diagnostic-router-mount.png'),fullPage:true});
  return result;
}

async function route(page,id,label){
  const el='#nav-'+id;await page.waitForSelector(el,{state:'visible',timeout:15000});await page.click(el);await page.waitForFunction(id=>window.CX?.session?.view===id,id,{timeout:15000});await page.waitForFunction(()=>String(document.getElementById('view')?.innerText||'').trim().length>0,null,{timeout:15000});
  ...
  ensure(r.currentView===id&&r.navVisible&&r.textLength>0,'M8_ROUTE_'+id+'_INVALID');r.viewDigest=hash(r.textPrefix);delete r.textPrefix;const f=path.join(outDir,'screenshots',id+'.png');await page.screenshot({path:f,fullPage:true});r.screenshot='screenshots/'+id+'.png';return r;
}
...
async function run(){
  const browser=await chromium.launch({headless:true,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage']});const context=await browser.newContext({viewport:{width:1440,height:1000},ignoreHTTPSErrors:true,serviceWorkers:'block'});const page=await context.newPage();
  const diag={consoleErrors:[],pageErrors:[],requestFailures:[],httpErrors:[]};let last=null,shellRecovery=null;
  ...
  try{
    await page.goto(root+'/',{waitUntil:'domcontentloaded',timeout:60000});await page.waitForSelector('.role-btn[data-role="admin"]',{state:'visible',timeout:30000});const entry=new URL(page.url());ensure(entry.searchParams.get('cxProtectedRuntime')==='YES_PAULA_20260730_PROTECTED_DEV'&&entry.searchParams.get('cxHumanFullVisual')==='YES_PAULA_20260731_FULL_PROFILE_DEV'&&!entry.searchParams.has('cxTechnicalAuthE2E'),'M8_ENTRY_LANE_FLAGS_INVALID');
    await page.click('.role-btn[data-role="admin"]');await page.waitForFunction(()=>document.getElementById('loginForm')?.dataset.selectedRole==='admin',null,{timeout:10000});await page.fill('#lgUser',cred.staff.login);await page.fill('#lgPass',cred.staff.password);await page.press('#lgPass','Enter');await waitReady(page,'m8_initial');await page.waitForTimeout(1000);last=await state(page,'m8_initial');validate(last,'M8_INITIAL');
    if(!last.shell.navDashboardVisible){shellRecovery=await shellDiagnosticRecovery(page,last);const after=await state(page,'m8_after_diagnostic_mount');const cls=!last.shell.railExists?'SHELL_ROOT_REPLACED':last.shell.emptyGuardActive||last.shell.emptyProject?'EMPTY_SHELL_GUARD_VISIBLE':last.shell.navCount===0?'RAIL_PRESENT_NAV_NOT_MOUNTED':'NAV_DASHBOARD_NOT_VISIBLE';throw new Error('M8_VISIBLE_SHELL_NOT_READY_'+cls+'_'+clean(JSON.stringify({before:last.shell,backendLast:last.backendLast,corte4:last.corte4,dataMode:last.dataMode,dataStatus:last.dataStatus,dataBlocked:last.dataBlocked,recovery:shellRecovery,after:after.shell})));}
    const routes=[];for(const [id,label] of [['dashboard','Dashboard Operativo'],['proyectos','Proyectos'],['visitas','Visitas'],['postulaciones','Postulaciones'],['cert','Certificación'],['financiero','Dashboard Financiero'],['aprendizaje','Academia']])routes.push(await route(page,id,label));
    ...
    const result={schemaVersion:'cxorbia.m8.human-validation-runtime-readonly.v3',generatedAt:new Date().toISOString(),decision:'PASS_M8_HUMAN_BROWSER_LIVE_HR_RECONCILIATION_READONLY',action:ACTION,devRoot:root,principal:{role:last.role,namespace:last.namespace,tenantId:last.tenantId,projectId:last.currentProjectId,membershipVerified:last.membershipVerified},runtimeSourceMode:'live_hr_runtime',liveHr:{periods:a2.periods,visits:a2.visits,shoppers:a2.shoppers,firstPeriod:a2.firstPeriod,latestPeriod:a2.latestPeriod,sourceRevision:a2.sourceRevision,sourceReadAt:a2.sourceReadAt,liveHrFetchAttempt:a2.liveHrFetchAttempt},visibleShell:{initial:last.shell,diagnosticRecoveryRequired:false},routes,financeBenefitsRoleSplit:split,freshReconciliation:{first:r1?.result||r1,second:r2?.result||r2,idempotency:idem},sourceLock:{buildId:a2.buildId,...a2.sourceLock},diagnostics:{consoleErrorCount:diag.consoleErrors.length,consoleErrors:diag.consoleErrors.slice(0,30),pageErrorCount:diag.pageErrors.length,sameOriginHttpErrorCount:diag.httpErrors.length,sameOriginRequestFailureCount:fails.length},safety:{hostingDeploys:0,cloudRunDeploys:0,authWrites:0,firestoreWrites:0,hrWrites:0,rulesWrites:0,storageWrites:0,makeCalls:0,geminiCalls:0,paymentWrites:0,merge:false,production:false},credentialsExposed:false,tokensExposed:false};persist(result);await page.evaluate(async()=>{try{await window.CX?.backendAuth?.signOut?.();}catch{}});return result;
  }catch(error){if(!last)last=await state(page,'failure').catch(()=>null);const result={schemaVersion:'cxorbia.m8.human-validation-runtime-readonly.failure.v3',generatedAt:new Date().toISOString(),decision:'FAIL_M8_HUMAN_BROWSER_LIVE_HR_RECONCILIATION_READONLY',action:ACTION,devRoot:root,error:clean(error?.stack||error?.message||error),lastState:last,shellDiagnosticRecovery:shellRecovery,diagnostics:{consoleErrors:diag.consoleErrors.slice(0,30),pageErrors:diag.pageErrors.slice(0,20),sameOriginHttpErrors:diag.httpErrors.slice(0,20),requestFailures:diag.requestFailures.filter(x=>x.url.startsWith(new URL(root).origin)).slice(0,20)},safety:{hostingDeploys:0,cloudRunDeploys:0,authWrites:0,firestoreWrites:0,hrWrites:0,rulesWrites:0,storageWrites:0,makeCalls:0,geminiCalls:0,paymentWrites:0,merge:false,production:false,diagnosticDomMountOnly:shellRecovery?.attempted===true},credentialsExposed:false,tokensExposed:false};persist(result);throw error;}finally{await context.close().catch(()=>{});await browser.close().catch(()=>{});}
}

try{const r=await run();console.log(JSON.stringify(r));}catch(e){if(!fs.existsSync(outputFile))persist({schemaVersion:'cxorbia.m8.human-validation-runtime-readonly.failure.v3',generatedAt:new Date().toISOString(),decision:'FAIL_M8_HUMAN_BROWSER_LIVE_HR_RECONCILIATION_READONLY',action:ACTION,error:clean(e?.stack||e?.message||e),safety:{hostingDeploys:0,cloudRunDeploys:0,authWrites:0,firestoreWrites:0,hrWrites:0,rulesWrites:0,storageWrites:0,makeCalls:0,geminiCalls:0,paymentWrites:0,merge:false,production:false},credentialsExposed:false,tokensExposed:false});console.error(fs.readFileSync(outputFile,'utf8'));process.exitCode=1;}
