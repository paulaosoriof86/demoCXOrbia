...
if(bundle.schemaVersion!=='cxorbia.legacy-credential-hash-bundle.v1'||bundle.targetProjectId!==expectedProject||bundle.tenantId!==tenantId||bundle.canonicalProjectId!==canonicalProjectId)throw new Error('bundle_contract_mismatch');
...
const counters={inputRecords:0,eligible:0,shopperEligible:0,userEligible:0,shopperExactLegacyMatch:0,holds:{bad_record:0,bad_hash:0,duplicate_login:0,shopper_legacy_missing:0,shopper_legacy_ambiguous:0,unknown_user_role:0,auth_uid_collision:0,auth_email_collision:0}};
const seenLogin=new Set();const roleTargets={};
const plan=[];
for(const r of Array.isArray(bundle.records)?bundle.records:[]){
 ...
 if(seenLogin.has(r.normalizedLogin)){counters.holds.duplicate_login++;continue}seenLogin.add(r.normalizedLogin);
 const digest=crypto.createHash('sha256').update(`${tenantId}\0${r.normalizedLogin}`,'utf8').digest('hex');
 ...
 const claims={tenantId,role,projectId:canonicalProjectId,projectIds:[canonicalProjectId]};
 if(shopperId)claims.shopperId=shopperId;
 plan.push({uid,internalEmail,passwordHashHex:r.passwordHashHex,claims,kind:r.kind});
 counters.eligible++;if(r.kind==='shopper')counters.shopperEligible++;else counters.userEligible++;roleTargets[role]=(roleTargets[role]||0)+1;
}
const summary={schemaVersion:'cxorbia.corte6.credential-handoff-dryrun.v1',generatedAt:new Date().toISOString(),projectId:expectedProject,tenantId,canonicalProjectId,keyFingerprintSha256:pub.fingerprintSha256,decision:counters.eligible>0?'READY_FOR_EXACT_AUTH_IMPORT_AUTHORIZATION':'HOLD_NO_ELIGIBLE_CREDENTIALS',legacySourceSummary:{shoppers:bundle.records.filter(r=>r.kind==='shopper').length,users:bundle.records.filter(r=>r.kind==='user').length,localHolds:bundle.holds||{}},providerInventory:{currentAuthUsers:totalAuth,canonicalShopperDocs:shopperSnap.size,canonicalProfilesWithLegacyShopperId:byLegacy.size},dryRun:counters,targetRoleCounts:roleTargets,plannedAuthImportMax:counters.eligible,collisionPolicy:'FAIL_CLOSED_NO_OVERWRITE',identifierContract:'visible username/password -> deterministic internal Firebase email',hashContract:{algorithm:'SHA256',rounds:1},authorizationRequiredBeforeWrite:true,safety:{providerWrites:0,authWrites:0,firestoreWrites:0,rulesDeploys:0,hostingDeploys:0,storageWrites:0,hrWrites:0,legacyWrites:0,production:false,merge:false,piiExported:false,loginIdentifiersExported:false,passwordHashesExported:false,secretsExported:false}};
...
console.log(JSON.stringify({decision:summary.decision,plannedAuthImportMax:summary.plannedAuthImportMax,shopperExactLegacyMatch:counters.shopperExactLegacyMatch,holds:counters.holds,targetRoleCounts:roleTargets,providerWrites:0,piiExported:false,passwordHashesExported:false}));
