...
const requiredTabs={GT:'AGOSTO 26',HN:'AGOSTO 26 HN'};
...
const blockers=[];
if(tabPresence.GT&&sourceByCountry.GT.length!==expectedByCountry.GT)blockers.push(`august_gt_count_drift:${sourceByCountry.GT.length}`);
if(tabPresence.HN&&sourceByCountry.HN.length!==expectedByCountry.HN)blockers.push(`august_hn_count_drift:${sourceByCountry.HN.length}`);
...
if(missingRequiredTabs){decision='HOLD_AUGUST_REQUIRED_PROVIDER_TABS_MISSING';releaseReadiness='SOURCE_TABS_MISSING';}
else if(!blockers.length&&acceptedNew.length===44){decision='PASS_AUGUST_FULL_DELTA_TECH_READY';releaseReadiness=operational.unassigned>0?'HAS_UNASSIGNED_VISITS_FOR_PUBLICATION':'NO_UNASSIGNED_VISITS_IN_ACCEPTED_SOURCE';}
const report={schemaVersion:'cxorbia.august-delta-readonly-plan.v4',generatedAt:new Date().toISOString(),decision,releaseReadiness,target:{firebaseProjectId:expectedProject,tenantId,projectId,periodKey},contract:{expectedByCountry,requiredTabs,doNotRepeatHistoricalWrites:1406,deltaOnly:true},providerTabRegistry:{observedAt:registry.observedAt,monthlyTabs:registryTabs.size,tabPresence,requiredAugustTabsPresent:tabPresence.GT&&tabPresence.HN},source:{generatedAt:source.generatedAt||null,accessMode:source.source?.accessMode||null,sourceSafe:true,tabRegistryEnforced:source.source?.tabRegistryEnforced===true,phantomTabsRejected:source.source?.phantomTabsRejected||[],observedAugust:{GT:sourceByCountry.GT.length,HN:sourceByCountry.HN.length,total:august.length}},countryGate:missingRequiredTabs?{status:'NOT_APPLICABLE_UNTIL_PROVIDER_TABS_EXIST'}:{decision:country?.decision||null,GT:countryResults[requiredTabs.GT]||null,HN:countryResults[requiredTabs.HN]||null},identityResolution:{canonicalMappingEntries:identityMap.size,requiredSourceShopperRefs:sourceShopperRefs.length,mappedSourceShopperRefs:mappedSourceRefs.length,unmappedSourceShopperRefs:unmappedSourceRefs.length,targetShopperProfiles:targetShopperIds.length,existingTargetShopperProfiles:existingTargetShopperIds.size,missingTargetShopperProfiles:missingTargetShopperIds.length},providerCompare:{currentCanonicalVisits:currentVisits.size,periodAlreadyExists:periodSnap.exists,acceptedCandidates:accepted.length,newVisitWritesPlanned:acceptedNew.length,alreadyExistingAccepted:acceptedExisting.length},operational,holds,blockers,exactDelta:{acceptedCountries:uniq(acceptedNew.map(v=>v.country)),visitIds:acceptedNew.map(v=>v.id).sort(),hrRowIds:acceptedNew.map(v=>v.hrRowId).sort(),shopperMappings:mappedSourceRefs.map(ref=>({sourceShopperRef:ref,targetShopperId:identityMap.get(ref).targetShopperId,targetKind:identityMap.get(ref).targetKind})),excludedCountries:holds.map(h=>h.country)},safety:{readOnly:true,providerReads:true,hrWrites:0,firestoreWrites:0,authWrites:0,rulesDeploys:0,hostingDeploys:0,production:false,merge:false,pii:false,secrets:false}};
fs.mkdirSync('app/docs/evidence',{recursive:true});fs.writeFileSync(outPath,JSON.stringify(report,null,2)+'\n','utf8');console.log(JSON.stringify({decision,releaseReadiness,providerTabRegistry:report.providerTabRegistry,source:report.source.observedAugust,providerCompare:report.providerCompare,operational,holds,blockers,safety:report.safety}));
