...
function safeRows(rows){return rows.map(v=>({id:String(v.id||v.visitId||''),hrRowId:String(v.hrRowId||''),sourceTab:String(v.sourceTab||''),sourceRow:Number(v.sourceRow||0),country:String(v.country||v.pais||''),shopperId:v.shopperId?String(v.shopperId):null}));}
...
const acceptedShopperIds=uniq(acceptedNew.map(v=>v.shopperId));const shopperRefs=acceptedShopperIds.map(id=>db.collection('tenants').doc(tenantId).collection('shoppers').doc(id));const shopperSnaps=[];for(let i=0;i<shopperRefs.length;i+=150)shopperSnaps.push(...await db.getAll(...shopperRefs.slice(i,i+150)));const existingShopperIds=new Set(shopperSnaps.filter(s=>s.exists).map(s=>s.id));const missingShopperIds=acceptedShopperIds.filter(id=>!existingShopperIds.has(id));
...
if(missingShopperIds.length)blockers.push(`accepted_visit_shopper_profile_gap:${missingShopperIds.length}`);
const hnHold=!!hnGate&&Number(hnGate.mismatchCount||0)>0;
let decision='HOLD_AUGUST_DELTA_PLAN';
if(!blockers.length&&acceptedNew.length===expectedByCountry.GT&&hnHold)decision='PASS_AUGUST_GT34_DELTA_READY__HN_HOLD_SOURCE_COUNTRY_MISMATCH';
else if(!blockers.length&&!hnHold&&acceptedNew.length===expectedByCountry.GT+expectedByCountry.HN)decision='PASS_AUGUST_FULL_DELTA_READY';
const report={schemaVersion:'cxorbia.august-delta-readonly-plan.v1',generatedAt:new Date().toISOString(),decision,target:{firebaseProjectId:expectedProject,tenantId,projectId,periodKey},contract:{expectedByCountry,doNotRepeatHistoricalWrites:1406,deltaOnly:true},source:{generatedAt:source.generatedAt||null,accessMode:source.source?.accessMode||null,sourceSafe:true,observedAugust:{GT:sourceByCountry.GT.length,HN:sourceByCountry.HN.length,total:august.length}},countryGate:{decision:country.decision,GT:gtGate?{mismatchCount:gtGate.mismatchCount,countryCounts:gtGate.countryCounts}:null,HN:hnGate?{mismatchCount:hnGate.mismatchCount,countryCounts:hnGate.countryCounts}:null},providerCompare:{currentCanonicalVisits:currentVisits.size,periodAlreadyExists:periodSnap.exists,acceptedCandidates:accepted.length,newVisitWritesPlanned:acceptedNew.length,alreadyExistingAccepted:acceptedExisting.length,requiredDistinctShopperProfiles:acceptedShopperIds.length,existingRequiredShopperProfiles:existingShopperIds.size,missingRequiredShopperProfiles:missingShopperIds.length},holds,blockers,exactDelta:{acceptedCountries:uniq(acceptedNew.map(v=>v.country)),visitIds:acceptedNew.map(v=>v.id).sort(),hrRowIds:acceptedNew.map(v=>v.hrRowId).sort(),excludedCountries:holds.map(h=>h.country)},safety:{readOnly:true,providerReads:true,hrWrites:0,firestoreWrites:0,authWrites:0,rulesDeploys:0,hostingDeploys:0,production:false,merge:false,pii:false,secrets:false}};
fs.mkdirSync('app/docs/evidence',{recursive:true});fs.writeFileSync(outPath,JSON.stringify(report,null,2)+'\n','utf8');console.log(JSON.stringify({decision,source:report.source.observedAugust,providerCompare:report.providerCompare,holds,blockers,safety:report.safety}));
