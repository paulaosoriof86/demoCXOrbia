...
const CONTRACT = 'backend/contracts/c6-direct-trusted-runner-dev-v2.json';
...
function fail(message, details = {}) {
  process.stdout.write(`${JSON.stringify({
    decision: 'HOLD_C6_DIRECT_RUNNER_SOURCE_GATE_V2',
    message,
...
const falseSafetyKeys = [
  ...
  'storageWrites', 'providerReads', 'providerWrites', 'hostingDeploy', 'merge',
  'production', 'make', 'gemini', 'payments'
];

const contractOk =
  contract.schemaVersion === 'cxorbia.c6.direct-trusted-runner.dev.v2' &&
  ...
  contract.sourceIdentity?.mode === 'github.event.pull_request.head.sha' &&
  contract.sourceIdentity?.checkout === 'PR_HEAD_SHA' &&
  contract.sourceIdentity?.validation === 'PR_HEAD_SHA' &&
  contract.sourceIdentity?.imageTag === 'PR_HEAD_SHA' &&
  contract.sourceIdentity?.forbiddenComparison === 'GITHUB_SHA' &&
  ...
  contract.deployment?.rollbackRequired === true &&
...
process.stdout.write(`${JSON.stringify({
  decision: 'PASS_C6_DIRECT_RUNNER_SOURCE_GATE_V2',
  providerBoundaryAllowed: false,
