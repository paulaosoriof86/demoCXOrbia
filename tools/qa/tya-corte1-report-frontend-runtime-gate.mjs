...
function run({role='director',period={id:'cinepolis-2026-07',periodKey:'2026-07',periodo:'JUL 2026'},scopeSucursal=null,country=''}){
  ...
  const colName=index=>{let n=index+1,out='';while(n>0){const r=(n-1)%26;out=String.fromCharCode(65+r)+out;n=Math.floor((n-1)/26);}return out;};
  const XLSX={utils:{book_new:()=>({sheets:[]}),aoa_to_sheet:data=>({data}),json_to_sheet:data=>({data}),encode_range:range=>`${colName(range.s.c)}${range.s.r+1}:${colName(range.e.c)}${range.e.r+1}`,book_append_sheet:(wb,sheet,name)=>wb.sheets.push({name,sheet})},writeFile:(wb,filename)=>exports.excel.push({wb,filename})};
  class PptxGenJS{constructor(){this.slides=[];this.ShapeType={rect:'rect'};this.ChartType={bar:'bar'};}defineLayout(){}set layout(v){}addSlide(){const s={addShape:()=>{},addText:()=>{},addChart:()=>{},addTable:()=>{}};this.slides.push(s);return s;}writeFile({fileName}){exports.ppt.push({fileName,slides:this.slides});return Promise.resolve();}}
...
c=run({role:'director',country:'GT'});c.click('executive_operational_summary','PDF');assert.equal(c.exports.prints,1);c.click('executive_operational_summary','PPT');assert.equal(c.exports.ppt.at(-1).slides.length,5);result.exports={pdfPrints:1,pptSlides:5};
