...
if(!SOURCE_DIR)throw new Error('SOURCE_FAILURE:CERTIFICATION_SOURCE_DIR_MISSING');
const credentialMod=await import(pathToFileURL(path.join(SOURCE_DIR,'backend/runtime/cxorbia-shopper-command-provider-v1.mjs')).href);
const cred=p=>{const c=credentialMod.shopperCredentialRule(p);return c?.ok?c:null;};
...
const CRED_RULE=credentialMod.CREDENTIAL_RULE_VERSION,PASSWORD_PROOF=credentialMod.CREDENTIAL_PASSWORD_PROOF_VERSION;
...
try{
 ...
 {const {c,p}=await ctx();await custom(p,staff,'admin');const summary=await p.evaluate(projectId=>{const list=Array.isArray(window.CX?.data?.shoppers)?window.CX.data.shoppers:[],platform=list.filter(x=>x?.__platformOnlyProfile===true),nonPlatform=list.filter(x=>x?.__platformOnlyProfile!==true),ids=list.map(x=>String(x?.id||x?.shopperId||'')).filter(Boolean),badFlags=platform.filter(x=>x?.__hrIdentityPresent!==false||x?.__hrOwnedOperational!==false||String(x?.identityAuthority||'')!=='platform_created_project_scoped'||x?.identityReviewRequired!==true||String(x?.identityReviewReason||'')!=='no_exact_hr_crosswalk').length,crossProject=platform.filter(x=>!Array.isArray(x?.projectIds)||!x.projectIds.map(String).includes(String(projectId))).length;return{visits:window.CX?.data?._visitas?.length||0,shoppers:list.length,hrShoppers:nonPlatform.length,platformOnly:platform.length,platformBadFlags:badFlags,platformCrossProject:crossProject,duplicateShopperIds:ids.length-new Set(ids).size,periods:window.CX?.data?.projects?.length||0,project:String(window.CX?.data?.currentProjectId||''),revision:String(window.CX?.data?.previewMeta?.sourceRevision||''),protectedNames:list.filter(x=>/shopper protegido/i.test(String(x?.nombre||x?.name||''))).length,realNames:list.filter(x=>String(x?.nombre||x?.name||'').trim()&&!/shopper protegido/i.test(String(x?.nombre||x?.name||''))).length}},PROJ);if(summary.visits!==visits.length||summary.hrShoppers!==shoppers.length||summary.shoppers!==shoppers.length+summary.platformOnly||summary.platformBadFlags!==0||summary.platformCrossProject!==0||summary.duplicateShopperIds!==0||summary.periods!==periods.length||summary.project!==PROJ)issues.push({classification:'MAPPING_FAILURE',code:'ADMIN_COMPOSED_SHOPPER_CONTRACT_MISMATCH',summary,expected:{visits:visits.length,hrShoppers:shoppers.length,totalFormula:'hrShoppers+platformOnly',platformBadFlags:0,platformCrossProject:0,duplicateShopperIds:0,periods:periods.length,project:PROJ}});if(summary.realNames<1||summary.protectedNames>0)issues.push({classification:'MAPPING_FAILURE',code:'ADMIN_REAL_SHOPPER_IDENTITY_NOT_RENDERABLE',summary});
