...
const accepted=all.filter(x=>x.currentAccepted),history=all.filter(x=>x.h>0&&x.co),acceptedHistory=history.filter(x=>x.currentAccepted);if(!all.length)throw new Error('AUTH_FAILURE:NO_CREDENTIAL_PROVEN_ACTIVE_SHOPPER');if(!history.length)throw new Error('MAPPING_FAILURE:NO_CREDENTIAL_PROVEN_SHOPPER_WITH_HR_HISTORY');if(!acceptedHistory.length)throw new Error('AUTH_FAILURE:NO_CURRENT_LEGAL_ACCEPTED_CREDENTIAL_PROVEN_SHOPPER_WITH_HR_HISTORY');
...
try{
 const acceptedOne=acceptedHistory[0];
 {const {c,p}=await ctx();try{
   const login=await visible(p,acceptedOne);
   if(login.legalModal)throw new Error('CURRENT_LEGAL_ACCEPTED_SHOPPER_SHOWED_LEGAL_MODAL');
