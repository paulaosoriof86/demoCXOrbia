#!/usr/bin/env node
/* CXOrbia V57 · Apply AI settings metadata in DEV. Requires explicit local confirmation. */
...
const REQUIRED = 'AUTORIZO_CARGAR_AI_SETTINGS_V57_FIRESTORE_DEV';
...
if(!input){
  console.error('Uso: node apply-ai-settings-write-plan-dev.mjs <write-plan.json>');
  process.exit(2);
}
...
if(!email || !password){
  console.error('Faltan variables DEV locales.');
  process.exit(2);
}
