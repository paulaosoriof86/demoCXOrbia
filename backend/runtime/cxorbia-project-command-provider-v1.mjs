...
export function createProjectCommandProvider({auth,db,policy}={}){
  ...
  return Object.freeze({
    ...
    async execute(token,command={}){
      ...
      if(!str(command.periodId||command.payload?.periodId))return blocked(command,'PROJECT_PERIOD_SCOPE_REQUIRED');
      ...
      try{return await db.runTransaction(async tx=>{
        ...
        tx.set(receipt,{status:'committed',commandDigest:digest,projectId,periodId:command.periodId||payload.periodId||null,commandType:command.commandType,providerAck:true,actorUid:who.uid,updatedAt:now()});providerWrites++;
